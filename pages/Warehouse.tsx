...
const Warehouse: React.FC<WarehouseProps> = ({
  ...
  const reversedWarehouses = useMemo(
    () => [...warehouses].reverse(),
    [warehouses]
  );
  const totalItems = reversedWarehouses.length;
  const paginatedWarehouses = reversedWarehouses.slice(
                      ...
                      {warehouse.type === 'คลัง' && (
                        ...
                        >
                          <div className="relative">
                            ...
                            />
                            <div
                              className={`block w-14 h-8 rounded-full ${warehouse.status === Status.Approved ? 'bg-primary' : 'bg-slate-300'}`}
                            ></div>
                            <div
                              className={`dot absolute left-1 top-1 bg-white w-6 h-6 rounded-full transition-transform ${warehouse.status === Status.Approved ? 'transform translate-x-6' : ''}`}
                            ></div>
                          </div>
                        </label>
                      )}
