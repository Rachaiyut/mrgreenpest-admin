...
} from '../libs/common/interface/entity/app.interface';
import {
  MOCK_USERS,
  MOCK_WAREHOUSES,
  MOCK_SUPPLIERS,
...
interface DataContextType {
  ...
  productReturns: ProductReturnType[];
  returnToSuppliers: ReturnToSupplierType[];
  invoices: Invoice[];
  receipts: Receipt[];
...
export const DataProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const updateWarehouseLimits = useCallback(
    (id: string, limits: { min: number; max: number }[]) => {
      setWarehouses((prev) =>
        prev.map((w) => (w.id === id ? { ...w, withdrawalLimits: limits } : w))
      );
    },
    []
  );
        ...
        return prev.map((w) =>
          w.userId === data.userId
            ? {
                ...w,
                balance: newBalance,
                transactions: [newTransaction, ...w.transactions],
              }
            : w
        );
      });
  ...
  const handleReviseQuotation = useCallback(
    (originalId: string, changes: Partial<Quotation>) => {
      setQuotations((prevQuotations) => {
        ...
        const relatedQuotations = prevQuotations.filter((q) =>
