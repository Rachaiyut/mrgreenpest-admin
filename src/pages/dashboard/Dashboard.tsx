    ...
    <div className="bg-[#eaf7f0]">
      <div className="p-4 sm:p-6 space-y-5 sm:space-y-6 max-w-[1600px] mx-auto">
        ...
        <div className="flex gap-2 flex-wrap">
          <QuickAction onClick={() => navigate('/jobs')} label="สร้างงาน" iconPath="M12 4v16m8-8H4" />
          <QuickAction onClick={() => navigate('/invoices')} label="สร้างใบแจ้งหนี้" iconPath="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
        ...
        {openClosures.length > 0 && (
          <div
            ...
            className="bg-amber-50 border border-amber-200 rounded-3xl p-4 flex items-center justify-between cursor-pointer hover:bg-amber-100 transition-colors"
          >
                ...
                {pendingActions.items.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className="w-full flex items-center justify-between p-3 rounded-2xl bg-slate-50 hover:bg-emerald-50 transition-colors group"
                    >
            ...
            {overdueInvoices.items.length > 0 && (
              <div className="space-y-1.5 max-h-32 overflow-y-auto">
                {overdueInvoices.items.slice(0, 5).map((inv: DashboardInvoiceItem) => (
                  <div key={inv.id} className="flex items-center justify-between text-xs p-2 bg-rose-50 rounded-lg">
...
const Card: React.FC<CardProps> = ({ children, className = '', onClick, hover }) => (
  <div
    onClick={onClick}
    className={`bg-white rounded-3xl border border-emerald-50 shadow-[0_2px_12px_rgba(16,185,129,0.04)] p-4 sm:p-5 ${
...
const KpiCard: React.FC<KpiCardProps> = ({ title, value, change, changeDir, footer, accent, isCountChange }) => {
  ...
  return (
    <div
      className={`relative bg-white rounded-3xl border border-emerald-50 shadow-[0_2px_12px_rgba(16,185,129,0.04)] p-5 overflow-hidden before:content-[''] before:absolute before:top-0 before:left-0 before:w-1 before:h-full ${ringColor[accent]}`}
    >
        ...
        <div className="space-y-2">
          {payments.items.map((item: DashboardPaymentMethodItem) => {
            ...
            return (
              <div
                key={item.method}
                className={`flex items-center justify-between p-3 rounded-2xl border ${accentBg[meta.color] || accentBg.slate}`}
              >
        ...
        <div className="lg:col-span-3 grid grid-cols-1 sm:grid-cols-3 gap-3">
          {buckets.map((b) => (
            <button
              ...
              className={`${b.bg} ${b.text} border rounded-2xl p-4 text-left hover:shadow-sm transition-all`}
            >
  ...
  <Card>
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2.5">
      {vehicles.map((v) => {
        ...
        return (
          <div key={v.id} className={`${meta.bg} ${meta.text} border rounded-2xl p-3`}>
  ...
  <Card>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
      {vehicles.map((v, idx) => {
        ...
        return (
          <div key={v.id} className="border border-slate-100 rounded-2xl p-4 bg-slate-50/30">
            ...
            <div className="grid grid-cols-4 gap-1.5 mb-3 text-center">
              <div className="bg-amber-50 rounded-lg px-1 py-1.5">
              ...
              <div className="bg-emerald-50 rounded-lg px-1 py-1.5">
              ...
              <div className="bg-rose-50 rounded-lg px-1 py-1.5">
