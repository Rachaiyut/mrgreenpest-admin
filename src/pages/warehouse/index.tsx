...
import { PlusIcon, ManageIcon, PencilIcon, TrashIcon, EyeIcon, LimitIcon, NewWarehouseIcon, SearchIcon, FilterIcon } from '../../assets/icons/Icons';
...
const Warehouse: React.FC<WarehouseProps> = ({ warehouses, products, onCreateWarehouse, onUpdateWarehouse, onDeleteWarehouse, onUpdateWarehouseLimits, stockMap = {} }) => {
    ...
    const filteredWarehouses = useMemo(() => {
        return warehouses.filter(w => {
            ...
            if (statusFilter === 'active' && w.status !== Status.Approved) return false;
            if (statusFilter === 'inactive' && w.status === Status.Approved) return false;
    ...
    const stats = useMemo(() => {
        ...
        const active = warehouses.filter(w => w.status === Status.Approved).length;
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            ...
            if ((event.target as HTMLElement).closest('button[data-warehouse-id]')) return;
            setOpenDropdownId(null);
        };
    ...
    const actions = [
        ...
        { label: 'จำกัดการเบิก', icon: LimitIcon, action: handleSetLimits, condition: (w: WarehouseType) => w.type === 'รถ' },
        ...
        <>
            <div className="p-4 sm:p-6 lg:p-8 flex flex-col h-full bg-slate-50/50">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                    <div>
                        <h1 className="text-2xl font-bold text-slate-800 tracking-tight">คลังสินค้า</h1>
                        <p className="mt-1 text-sm text-slate-500">จัดการข้อมูลคลังสินค้าและรถบริการของบริษัท</p>
                    </div>
                    <div>
                        <Button onClick={() => setIsAddModalOpen(true)} variant="primary" className="shadow-sm">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                    ...
                    <div className="bg-white rounded-2xl p-4 border border-slate-100 shadow-sm flex items-center space-x-4 transition-all hover:shadow-md hover:border-emerald-200">
                        ...
                        <div>
                            ...
                            <h3 className="text-2xl font-bold text-slate-800">{stats.fixed}</h3>
                        </div>
                    </div>
                    ...
                    <div className="p-4 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        ...
                        <div className="flex bg-slate-100 p-1 rounded-lg self-start">
                            ...
                            <button
                                ...
                                className={`px-4 py-1.5 text-sm font-medium rounded-md transition-all ${activeTab === 'warehouse' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                                    }`}
                            >
                                คลังสินค้า
                            </button>
                            <button
                                ...
                                className={`px-4 py-1.5 text-sm font-medium rounded-md transition-all ${activeTab === 'vehicle' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                                    }`}
                            >
                                รถบริการ
                            </button>
                    ...
                    <div className="overflow-auto flex-grow">
                        <table className="min-w-full divide-y divide-slate-200">
                            <thead className="bg-slate-50 sticky top-0 z-10">
                                <tr>
                                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">ลำดับ</th>
                                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">รหัส</th>
                                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">ชื่อคลัง/รถ</th>
                                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">ประเภท</th>
                                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">ที่ตั้ง/ทะเบียน</th>
                                    <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-slate-500 uppercase tracking-wider">สถานะ</th>
                                    <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-slate-500 uppercase tracking-wider">ใช้งานคลังหลัก</th>
                            ...
                            <tbody className="bg-white divide-y divide-slate-200">
                                {paginatedWarehouses.length > 0 ? paginatedWarehouses.map((warehouse, index) => (
                                    <tr key={warehouse.id} className="hover:bg-slate-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-900">
                                            <div className="flex items-center">
                                                <div className={`h-8 w-8 rounded-full flex items-center justify-center mr-3 ${warehouse.type === 'คลัง' ? 'bg-blue-100 text-blue-600' : 'bg-amber-100 text-amber-600'}`}>
                                                    {warehouse.type === 'คลัง' ? <NewWarehouseIcon className="h-4 w-4" /> : <ManageIcon className="h-4 w-4" />}
                                                </div>
                                                {warehouse.name}
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500">
                                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${warehouse.type === 'คลัง' ? 'bg-blue-50 text-blue-700' : 'bg-amber-50 text-amber-700'}`}>
                                                {warehouse.type}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600">
                                            {warehouse.type === 'รถ' ? (
                                                <div className="flex flex-col">
                                                    ...
                                                    <span className="text-xs text-slate-400">{warehouse.brand} {warehouse.model}</span>
                                                </div>
                                            ) : (
                                                warehouse.location
                                            )}
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-center">
                                            ...
                                            <StatusBadge status={warehouse.status || Status.Draft} /> {/* Assuming Approved means Active */}
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-center">
                                            {warehouse.type === 'คลัง' ? (
                    ...
                    <div className="py-1">
                        {(() => {
                            ...
                            return actions.map(action => {
                                if (action.condition && !action.condition(warehouse)) return null;
                                return (
                                    <button
                                        key={action.label}
                                        onClick={() => action.action(warehouse)}
                                        className={`flex w-full items-center px-4 py-2 text-sm ${action.isDanger ? 'text-red-600 hover:bg-red-50' : 'text-slate-700 hover:bg-slate-100'
                                            }`}
                                    >
