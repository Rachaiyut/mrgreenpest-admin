...
import { PlusIcon, ShieldCheckIcon, ArrowLeftIcon, ManageIcon, EyeIcon, PencilIcon, TrashIcon, WalletIcon } from '../../assets/icons/Icons';
...
interface UsersProps {
    ...
    onCreateWalletTransaction: (userId: string, transactionData: Omit<WalletTransaction, 'id'>) => void;
}

const Users: React.FC<UsersProps> = ({ users, onCreateUser, onUpdateUser, onDeleteUser, defaultView = 'users', userWallets, onCreateWalletTransaction }) => {
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            ...
            if ((event.target as HTMLElement).closest('button[data-user-id], button[data-role-id]')) {
                return;
            }
            setOpenDropdownId(null);
        };
    ...
    return (
        <>
            <div className="p-4 sm:p-6 lg:p-8">
                                ...
                                <table className="min-w-full divide-y divide-slate-200">
                                    <thead className="bg-slate-50">
                                        <tr>
                                            ...
                                            <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase whitespace-nowrap">ชื่อ</th>
                                            <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase whitespace-nowrap">อีเมล</th>
                                            <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase whitespace-nowrap">โทรศัพท์</th>
                                            <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase whitespace-nowrap">บทบาท</th>
                                    ...
                                    <tbody className="bg-white divide-y divide-slate-200">
                                        {paginatedUsers.map((user, index) => (
                                            <tr key={user.id} className="hover:bg-slate-50">
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500">{(currentPage - 1) * itemsPerPage + index + 1}</td>
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <div className="flex items-center">
                                                        ...
                                                        <div className="ml-4">
                                                            <div className="text-sm font-medium text-slate-900">{user.name} ({user.nickname})</div>
                                                        </div>
                                                    </div>
                                                </td>
                ...
                {view === 'roles' && (
                    <>
                        <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                            <div>
                                <h1 className="text-3xl font-bold text-slate-800">จัดการบทบาท</h1>
                                <p className="mt-1 text-slate-600">สร้างและแก้ไขบทบาทผู้ใช้งานในระบบ</p>
                            </div>
                            <div className="flex items-center gap-2">
                                <Button
                        ...
                        <Card className="!p-0">
                            <div className="overflow-x-auto">
                                <table className="min-w-full divide-y divide-slate-200">
                                    <thead className="bg-slate-50">
                                        <tr>
                                            <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase whitespace-nowrap">ชื่อบทบาท</th>
                                            <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase whitespace-nowrap">จำนวนผู้ใช้งาน</th>
                                            <th scope="col" className="relative px-6 py-3">
                                                <span className="sr-only">จัดการ</span>
                                            </th>
                                        </tr>
                                    ...
                                    <tbody className="bg-white divide-y divide-slate-200">
                                        {Object.values(UserRole).map((role) => (
                                            <tr key={role} className="hover:bg-slate-50">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500">{roleCounts[role]}</td>
                                                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                    <div className="inline-block text-left">
                                                        <Button
                                                            data-role-id={role}
             ...
             {openDropdownId && dropdownPosition && (
                ...
                >
                    <div className="py-1" role="none">
                        {view === 'users' && userActions.map(action => (
                            <a
                                key={action.label}
                                href="#"
                                onClick={(e) => { 
                        ...
                        {view === 'roles' && (
                            <>
                                <a
                                    href="#"
                                    onClick={(e) => { e.preventDefault(); handleViewRoleDetails(openDropdownId as UserRole); }}
                                ...
                                <a
                                    href="#"
                                    onClick={(e) => { e.preventDefault(); setOpenDropdownId(null); setIsAddRoleModalOpen(true); }}
                                    className="flex items-center w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-100"
                                    role="menuitem"
                                >
                                ...
                                <Button
                                    ...
                                    title={roleCounts[openDropdownId as UserRole] > 0 ? 'ไม่สามารถลบบทบาทที่มีผู้ใช้งานได้' : 'ลบบทบาท'}
            ...
            <AddUserModal isOpen={isAddUserModalOpen} onClose={() => setIsAddUserModalOpen(false)} onCreateUser={onCreateUser} />
            <EditUserModal 
            ...
            <ConfirmationModal
                ...
                title="ยืนยันการลบ"
                message={<p>คุณแน่ใจหรือไม่ว่าต้องการลบผู้ใช้งาน <strong>{userToDelete?.name}</strong>? การกระทำนี้ไม่สามารถย้อนกลับได้</p>}
            ...
            <UserWalletModal
                ...
                wallet={userWallets.find(w => w.userId === selectedUserForWallet?.id) || { userId: selectedUserForWallet?.id || '', transactions: [] }}
