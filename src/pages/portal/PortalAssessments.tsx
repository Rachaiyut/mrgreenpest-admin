...
const statusLabels: Record<string, { label: string; cls: string }> = {
  ...
  APPOINTMENT: { label: 'นัดหมาย', cls: 'bg-blue-100 text-blue-800' },
  ...
  CANCELLED: { label: 'ยกเลิก', cls: 'bg-slate-100 text-slate-700' },
};

const PortalAssessments: React.FC = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        const res = await portalApi.getAssessments();
  ...
  const handleDownloadPdf = async (id: string, code?: string) => {
    ...
    setLoadingPdfId(id);
    try {
      await openPortalPdf('assessments', id, `${code || 'assessment'}-${id}.pdf`);
    } catch (error) {
  ...
  return (
    <div>
      <div className="mb-5 sm:mb-6 flex items-end justify-between gap-3">
        <div>
          <h2 className="text-xl sm:text-2xl font-extrabold text-slate-800 tracking-tight">ใบประเมิน</h2>
          <p className="text-slate-500 mt-0.5 text-sm">รายการใบประเมินบริการทั้งหมด</p>
        </div>
        <span className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-blue-50 text-blue-700 border border-blue-100">
          ทั้งหมด {assessments.length} รายการ
        </span>
      </div>
      ...
      {assessments.length === 0 ? (
        <div className="bg-white rounded-2xl border border-slate-200 p-8 sm:p-12 text-center shadow-sm">
          ...
          <p className="text-slate-600 font-medium">ยังไม่มีใบประเมิน</p>
          <p className="text-slate-400 text-sm mt-1">หลังจากเจ้าหน้าที่ประเมินพื้นที่ ใบประเมินจะแสดงที่นี่</p>
        </div>
      ) : (
        <>
          {/* Mobile cards */}
          <div className="md:hidden space-y-3">
            {assessments.map((item) => {
              const status = statusLabels[item.status] || null;
              return (
                <div
                  key={item.id}
                  ...
                  <div className="p-4 space-y-3">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <span className="text-[10px] font-semibold text-blue-600 uppercase tracking-wider">ใบประเมิน</span>
                        <p className="text-base font-bold text-slate-900 mt-0.5 truncate" title={item.code}>{item.code || '-'}</p>
                      </div>
                      {status && (
                    ...
                    <div className="pt-2 border-t border-slate-100 grid grid-cols-2 gap-3">
                      <div>
                        <p className="text-[11px] text-slate-400 font-medium">วันนัดหมาย</p>
                        <p className="text-sm text-slate-800 font-semibold mt-0.5">
                          {item.appointment_date ? dayjs(item.appointment_date).format('DD MMM YYYY') : '-'}
                        </p>
                      </div>
                      <div>
                        <p className="text-[11px] text-slate-400 font-medium">ยอดรวม</p>
                        <p className="text-sm text-slate-900 font-bold mt-0.5">
                          {item.total_price ? `${Number(item.total_price).toLocaleString('th-TH', { minimumFractionDigits: 2 })} ฿` : '-'}
                        </p>
                      </div>
                    </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200">
                    ...
                    <th className="text-left px-6 py-3 text-sm font-semibold text-slate-600">วันนัดหมาย</th>
                    <th className="text-center px-6 py-3 text-sm font-semibold text-slate-600">สถานะ</th>
                    <th className="text-right px-6 py-3 text-sm font-semibold text-slate-600">ยอดรวม</th>
                  ...
                  {assessments.map((item) => {
                    ...
                    return (
                      <tr key={item.id} className="border-b border-slate-100 hover:bg-slate-50">
                        ...
                        <td className="px-6 py-4 text-sm text-left text-slate-600">
                          {item.appointment_date ? dayjs(item.appointment_date).format('DD/MM/YYYY') : '-'}
                        </td>
                        ...
                        <td className="px-6 py-4 text-sm text-right font-medium text-slate-800">
                          {item.total_price ? `${Number(item.total_price).toLocaleString('th-TH', { minimumFractionDigits: 2 })} บาท` : '-'}
                        </td>
...
export default PortalAssessments;
