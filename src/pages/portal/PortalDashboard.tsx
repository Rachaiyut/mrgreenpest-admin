...
const PortalDashboard: React.FC = () => {
  ...
  const [counts, setCounts] = useState({ assessments: 0, quotations: 0, contracts: 0, receipts: 0, serviceReports: 0 });
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchCounts = async () => {
      try {
        const [assessmentsRes, quotationsRes, contractsRes, receiptsRes, reportsRes] = await Promise.all([
          portalApi.getAssessments(),
