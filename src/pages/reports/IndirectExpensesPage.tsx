...
const MOCK_EXPENSES: ExpenseItem[] = [
    { id: '1', category: 'ค่าสาธารณูปโภค', item: 'ค่าไฟฟ้า', amount: 4500, type: 'Fixed', wallet: 'กระเป๋า admin', date: '2025-12-05' },
    { id: '2', category: 'ค่าสาธารณูปโภค', item: 'ค่าน้ำประปา', amount: 350, type: 'Fixed', wallet: 'กระเป๋า admin', date: '2025-12-05' },
    { id: '3', category: 'ค่าสาธารณูปโภค', item: 'ค่าอินเทอร์เน็ต', amount: 799, type: 'Fixed', wallet: 'กระเป๋าหัวหน้าadmin', date: '2025-12-05' },
    { id: '4', category: 'ค่าเช่า', item: 'ค่าเช่าสำนักงาน', amount: 15000, type: 'Fixed', wallet: 'กระเป๋า CFO', date: '2025-12-01' },
    { id: '5', category: 'เงินเดือน', item: 'เงินเดือนพนักงาน', amount: 120000, type: 'Fixed', wallet: 'กระเป๋า CEO', date: '2025-12-25' },
    { id: '6', category: 'ค่าเดินทาง', item: 'ค่าน้ำมัน', amount: 2500, type: 'Variable', wallet: 'กระเป๋าหัวหน้าทีม1', date: '2025-12-10' },
    { id: '7', category: 'วัสดุสำนักงาน', item: 'กระดาษ A4', amount: 500, type: 'Variable', wallet: 'กระเป๋า admin', date: '2025-12-12' },
    { id: '8', category: 'ค่ารับรอง', item: 'เลี้ยงลูกค้า', amount: 3000, type: 'Variable', wallet: 'กระเป๋า CEO', date: '2025-12-15' },
    { id: '9', category: 'ค่าซ่อมบำรุง', item: 'ซ่อมแอร์', amount: 1200, type: 'Variable', wallet: 'กระเป๋า COO', date: '2025-12-20' },
    { id: '10', category: 'เบ็ดเตล็ด', item: 'เครื่องดื่ม', amount: 300, type: 'Variable', wallet: 'กระเป๋า Truewallet', date: '2025-12-22' },
];
...
const IndirectExpensesPage: React.FC = () => {
    ...
    const costTypeSummary = useMemo(() => {
        ...
        filteredExpenses.forEach(exp => {
            const existing = summary.find(s => s.type === exp.type && s.category === exp.category);
            if (existing) {
                existing.amount += exp.amount;
            } else {
                summary.push({ type: exp.type, category: exp.category, amount: exp.amount });
            }
        });
        ...
        <div className="space-y-6 animate-fade-in text-nowrap pb-20">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-slate-800">รายงานค่าใช้จ่ายทางอ้อม</h1>
                        <p className="text-slate-500 mt-1">สรุปค่าใช้จ่ายประเภทต่างๆ ประจำเดือน</p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                ...
                <Card className="!p-0 overflow-hidden shadow-sm border border-slate-200">
                    <div className="px-4 py-3 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
                        <h2 className="font-semibold text-slate-800">สรุปรายการแยกตามประเภท Cost</h2>
                    </div>
                                ...
                                {costTypeSummary.length > 0 ? (
                                    costTypeSummary.map((item, idx) => (
                                        <tr key={idx} className="hover:bg-slate-50">
                                            <td className="px-4 py-3 font-medium text-slate-600">{item.type}</td>
                                            <td className="px-4 py-3 text-slate-600">{item.category}</td>
                                            <td className="px-4 py-3 text-right font-medium text-slate-900">{item.amount.toLocaleString('th-TH', { minimumFractionDigits: 2 })}</td>
                                        </tr>
                                    ))
                                ) : (
                                    <tr><td colSpan={3} className="px-4 py-6 text-center text-slate-500">ไม่พบข้อมูล</td></tr>
                                )}
                                {costTypeSummary.length > 0 && (
                                    <tr className="bg-slate-50 font-bold border-t border-slate-200">
                                        <td colSpan={2} className="px-4 py-3 text-right text-slate-700">รวมทั้งหมด</td>
                                        <td className="px-4 py-3 text-right text-slate-900">{totalExpense.toLocaleString('th-TH', { minimumFractionDigits: 2 })}</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
                ...
                <Card className="!p-0 overflow-hidden shadow-sm border border-slate-200 h-fit">
                    <div className="px-4 py-3 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
                        <h2 className="font-semibold text-slate-800">สรุปรายการแยกตามกระเป๋า</h2>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm text-left">
                            <thead className="text-xs text-slate-700 uppercase bg-slate-50 border-b border-slate-200">
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {walletSummary.length > 0 ? (
                                    ...
                                    <tr><td colSpan={2} className="px-4 py-6 text-center text-slate-500">ไม่พบข้อมูล</td></tr>
                                )}
                                {walletSummary.length > 0 && (
                                    <tr className="bg-slate-50 font-bold border-t border-slate-200">
                                        <td className="px-4 py-3 text-right text-slate-700">รวมทั้งหมด</td>
                                        <td className="px-4 py-3 text-right text-slate-900">{totalExpense.toLocaleString('th-TH', { minimumFractionDigits: 2 })}</td>
                                    </tr>
                                )}
                            </tbody>
            ...
            <Card className="!p-0 overflow-hidden shadow-sm border border-slate-200">
                <div className="px-4 py-3 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
                    <h2 className="font-semibold text-slate-800">รายละเอียดค่าใช้จ่ายแต่ละหมวดหมู่</h2>
                </div>
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left">
                        <thead className="text-xs text-slate-700 uppercase bg-slate-50 border-b border-slate-200">
