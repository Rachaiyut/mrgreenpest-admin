    ...
    useEffect(() => {
        ...
        if (found) {
            ...
            setFormData({
                ...
                createdAt: found.createdAt ? new Date(found.createdAt).toISOString().substring(0, 10) : '',
                expiresAt: found.expiresAt ? new Date(found.expiresAt).toISOString().substring(0, 10) : '',
            });
        } else if (id) {
    ...
    const handleSave = (e: React.FormEvent) => {
        ...
        if (quotation) {
            const updatedData: Quotation = {
                ...
                status: isReviseMode ? Status.Revise : (formData.status || quotation.status),
            };
        ...
        <div className="p-6">
            <div className="mb-6 flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-2xl font-bold text-slate-800">
                        {isReviseMode ? `Revise ใบเสนอราคา: ${quotation.id}` : `แก้ไขสถานะ: ${quotation.id}`}
                    </h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="bg-white rounded-lg shadow-sm p-6 border border-slate-200">
                        <form onSubmit={handleSave} className="space-y-6">
                            <FormField label="ลูกค้า" htmlFor="customerName">
                                <Input id="customerName" type="text" value={formData.customerName || ''} readOnly className="bg-slate-100" />
                            </FormField>

                            {formData.assessmentId && (
                                <FormField label="อ้างอิงใบประเมิน" htmlFor="assessmentId">
                                    <Input id="assessmentId" type="text" value={formData.assessmentId} readOnly className="bg-slate-100" />
                                </FormField>
                            )}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <FormField label="สถานะ" htmlFor="status">
                                    <Select
                                        ...
                                        className={isReviseMode ? 'bg-slate-100 cursor-not-allowed' : ''}
                                    >
                                        {/* Original/Current Status Option if it's not in the allowed list (to show current state) */}
                                        {![Status.UnderReview, Status.PendingApproval, Status.Approved, Status.Closed].includes(formData.status as Status) && (
                ...
                <div className="lg:col-span-1">
                    <div className="bg-white rounded-lg shadow-sm p-6 border border-slate-200">
                        <h3 className="text-lg font-semibold text-slate-800 mb-4">ประวัติการแก้ไข (Version History)</h3>
                                    ...
                                    <li key={item.id}>
                                        <div className="relative pb-8">
                                            ...
                                            <div className="relative flex space-x-3">
                                                <div>
                                                    <span className={`h-8 w-8 rounded-full flex items-center justify-center ring-8 ring-white ${item.id === quotation.id ? 'bg-primary' : 'bg-slate-400'}`}>
                                                        <span className="text-white text-xs font-bold">{item.revision}</span>
                                                    </span>
                                                </div>
                                                <div className="flex min-w-0 flex-1 justify-between space-x-4 pt-1.5">
                                                    <div>
                                                        <p className={`text-sm ${item.id === quotation.id ? 'font-bold text-slate-900' : 'text-slate-500'}`}>
                                                            {item.id}
                                                        </p>
