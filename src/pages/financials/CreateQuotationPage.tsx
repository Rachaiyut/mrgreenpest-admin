...
import { Customer, Quotation, Assessment, Status, InstallmentPlan } from '../../types';
...
export const CreateQuotationPage: React.FC<CreateQuotationPageProps> = ({ onCreateQuotation, customers, assessments }) => {
    ...
    const [installments, setInstallments] = useState<Omit<InstallmentPlan, 'id' | 'status'>[]>([
        { term: 1, percentage: 50, amount: 0, description: 'มัดจำงวดแรก', dueDate: '' },
        { term: 2, percentage: 50, amount: 0, description: 'ชำระส่วนที่เหลือเมื่อเสร็จงาน', dueDate: '' }
    ]);

    const productMap = useMemo(() => new Map(MOCK_PRODUCTS.map(p => [p.id, p])), []);
    ...
    const availableAssessmentsForCustomer = useMemo(() => {
        ...
        return assessments.filter(a => a.customerId === selectedCustomerId && (a.status === Status.Completed || a.status === Status.Converted));
    ...
    const handleInstallmentChange = (index: number, field: keyof Omit<InstallmentPlan, 'id' | 'status'>, value: any) => {
        ...
        if (field === 'percentage') {
            ...
            newInstallments[index].percentage = numericTotal > 0 ? (amt / numericTotal) * 100 : 0;
        } else {
    ...
    const addInstallment = () => {
        setInstallments([
            ...installments,
            { term: installments.length + 1, percentage: 0, amount: 0, description: '', dueDate: '' }
        ]);
    };

    const removeInstallment = (index: number) => {
        if (installments.length <= 1) return;
        setInstallments(installments.filter((_, i) => i !== index).map((inst, i) => ({ ...inst, term: i + 1 })));
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (paymentMode === 'installments') {
            ...
            if (Math.abs(totalPct - 100) > 0.1) { // Floating point tolerance
                alert(`ผลรวมเปอร์เซ็นต์ต้องเท่ากับ 100% (ปัจจุบัน: ${totalPct.toFixed(2)}%)`);
                return;
            }
        }
        ...
        const newQuotation: Omit<Quotation, 'id'> = {
            ...
            revision: 1,
            paymentTerms: paymentMode === 'full' ? 'ชำระเต็มจำนวน' : 'แบ่งชำระเป็นงวด',
            installments: paymentMode === 'installments' ? installments.map((inst, i) => ({
                id: `inst-${Date.now()}-${i}`,
                ...inst,
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-6">
                            ...
                            {selectedCustomerId && (
                                <FormField label="อ้างอิงใบประเมิน (ไม่บังคับ)" htmlFor="assessment-select">
                                    <Select id="assessment-select" value={selectedAssessmentId} onChange={e => setSelectedAssessmentId(e.target.value)}>
                                        <option value="">-- ไม่อ้างอิงใบประเมิน --</option>
                                        {availableAssessmentsForCustomer.map(a => (
                                            <option key={a.id} value={a.id}>{a.id} - ยอดรวม: ฿{a.totalEstimatedCost.toLocaleString('th-TH', { minimumFractionDigits: 2 })}</option>
                                        ))}
                                    </Select>
                                </FormField>
                            )}
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <FormField label="วันที่สร้าง" htmlFor="created-at">
                                    <Input id="created-at" type="date" value={createdAt} onChange={e => setCreatedAt(e.target.value)} required />
                                </FormField>
                                <FormField label="หมดอายุวันที่" htmlFor="expires-at">
                                    <Input id="expires-at" type="date" value={expiresAt} onChange={e => setExpiresAt(e.target.value)} required />
                                </FormField>
                            </div>
                        </div>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
                                <label className="block text-sm font-medium text-slate-700 mb-3">เงื่อนไขการชำระเงิน</label>
                                <div className="flex gap-4 mb-4">
                                    <label className="flex items-center gap-2 cursor-pointer">
                                        ...
                                        />
                                        <span className="text-sm text-slate-700">ชำระเต็มจำนวน</span>
                                    </label>
                                    <label className="flex items-center gap-2 cursor-pointer">
                                        <input
                                            type="radio"
                                            ...
                                            className="text-primary focus:ring-primary"
                                        />
                                        <span className="text-sm text-slate-700">แบ่งชำระ (งวด)</span>
                                    </label>
                                </div>
                                ...
                                {paymentMode === 'installments' && (
                                    <div className="space-y-3">
                                        {installments.map((inst, idx) => (
                                            <div key={idx} className="flex gap-2 items-start bg-white p-2 rounded border border-slate-200 shadow-sm relative group">
                                                <div className="w-8 pt-2 text-center text-xs font-bold text-slate-500">{inst.term}</div>
                                                    ...
                                                    <div className="flex gap-2">
                                                        <div className="relative w-20">
                                                            <input
                                                                ...
                                                                onChange={e => handleInstallmentChange(idx, 'percentage', e.target.value)}
                                                            />
                                                        ...
                                                        <div className="relative flex-1">
                                                            ...
                                                            />
                                                            <span className="absolute right-2 top-1 text-xs text-slate-400">THB</span>
                                                        </div>
                                                    </div>
                                                </div>
                    ...
                    {selectedAssessment && (
                        <div className="mt-6 p-4 bg-slate-50 border rounded-lg space-y-4 text-sm max-h-60 overflow-y-auto">
                            <h4 className="text-base font-semibold text-slate-800">รายละเอียดใบประเมิน: {selectedAssessment.id}</h4>
                            <div className="space-y-3">
                                {selectedAssessment.workAreas.map((area, index) => (
                                    <div key={area.id} className="p-3 border bg-white rounded-md">
                                        <div className="flex justify-between items-start">
                                            <p className="font-semibold text-primary">พื้นที่ #{index + 1}: {area.name}</p>
                                            <p className="font-bold text-slate-700">฿{area.estimatedCost.toLocaleString('th-TH', { minimumFractionDigits: 2 })}</p>
                                        </div>
                                        <p className="text-xs text-slate-600 mt-1">บริการ: {area.serviceType.join(', ')}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    <div className="flex justify-end gap-3 pt-6 border-t mt-6">
                        <Button type="button" onClick={() => navigate('/quotations')} className="py-2 px-6" variant="outline">
                            ยกเลิก
                        </Button>
