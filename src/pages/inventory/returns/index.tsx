...
import { ProductReturn as ReturnType, Warehouse as WarehouseType, Product } from '../../../types';
...
const Returns: React.FC<ReturnsProps> = ({ returns, onCreateReturn, onUpdateReturn, onDeleteReturn, warehouses, products, stockMap }) => {
    ...
    const handleDropdownToggle = (event: React.MouseEvent<HTMLButtonElement>, returnId: string) => {
    ...
    return (
        <>
            <div className="p-4 sm:p-6 lg:p-8 flex flex-col h-full">
                <div className="flex-shrink-0 flex flex-wrap items-center justify-between gap-4 mb-6">
                    <div>
                        ...
                        <p className="mt-1 text-slate-600">จัดการการคืนสินค้าจากรถบริการกลับเข้าคลัง</p>
                    </div>
                        ...
                        <table className="min-w-full divide-y divide-slate-200">
                            <thead className="bg-slate-50 sticky top-0 z-10">
                                <tr>
                                    ...
                                    <th scope="col" className="px-4 py-2.5 text-left text-sm font-medium text-slate-600 uppercase">วันที่คืน</th>
                                    <th scope="col" className="px-4 py-2.5 text-left text-sm font-medium text-slate-600 uppercase">คืนจาก (รถ)</th>
                                    <th scope="col" className="px-4 py-2.5 text-left text-sm font-medium text-slate-600 uppercase">คืนเข้าคลัง</th>
                                ...
                                {paginatedReturns.map((item, index) => {
                                    ...
                                    return (
                                        <tr key={item.id} className="hover:bg-slate-50">
                                            ...
                                            <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-primary hover:underline cursor-pointer" onClick={() => handleViewDetails(item)}>{item.id}</td>
                                            <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-500">{formatThaiDate(item.createdAt)}</td>
                                            ...
                                            <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-500 text-center">{item.items.length}</td>
                                            <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-500">{item.createdBy}</td>
                        ...
                        {actions.map(action => (
                             <a
                                ...
                                onClick={(e) => { 
                                    ...
                                    if (!returnItem) { setOpenDropdownId(null); return; }
            ...
            <AddReturnModal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} onCreateReturn={onCreateReturn} returns={returns} warehouses={warehouses} products={products} stockMap={stockMap} />
            <EditReturnModal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} returnItem={returnToEdit} onUpdateReturn={onUpdateReturn} warehouses={warehouses} products={products} />
            <ReturnDetailsModal
            ...
            <ConfirmationModal
                ...
                title="ยืนยันการลบ"
                message={<p>คุณแน่ใจหรือไม่ว่าต้องการลบใบคืนสินค้า <strong>{returnToDelete?.id}</strong>? การกระทำนี้ไม่สามารถย้อนกลับได้</p>}
