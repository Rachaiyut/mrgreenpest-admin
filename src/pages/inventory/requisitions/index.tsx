...
const Requisitions: React.FC<RequisitionsProps> = () => {
    ...
    const [requisitionToDelete, setRequisitionToDelete] = useState<RequisitionType | null>(null);
    ...
    const filteredRequisitions = useMemo(() => {
        ...
        if (requesterId) {
            result = result.filter(item =>
                (typeof item.requester_id === 'string' && item.requester_id === requesterId) ||
                (typeof item.requester === 'object' && item.requester?.id === requesterId)
            );
        }
    ...
    const handleApprovalAction = (req: RequisitionType, action: 'approve' | 'reject') => {
        setSelectedRequisition(req);
    ...
    const actions = [
        { label: 'ดูรายละเอียด', icon: EyeIcon, isDanger: false, onClick: (req: RequisitionType) => handleViewDetails(req) },
        // { label: 'แก้ไข', icon: PencilIcon, isDanger: false, onClick: () => { } }, // Implement edit later
        { label: 'อนุมัติ', icon: EyeIcon, isDanger: false, onClick: (req: RequisitionType) => handleApprovalAction(req, 'approve') }, // Using EyeIcon as placeholder or need CheckIcon
        { label: 'ปฏิเสธ', icon: TrashIcon, isDanger: true, onClick: (req: RequisitionType) => handleApprovalAction(req, 'reject') },
        { label: 'ลบ', icon: TrashIcon, isDanger: true, onClick: (req: RequisitionType) => handleDelete(req) },
    ];
        ...
        <>
            <div className="p-4 sm:p-6 lg:p-8 flex flex-col h-full">
                <div className="flex-shrink-0 flex flex-wrap items-center justify-between gap-4 mb-6">
                    <div>
                        <h1 className="text-3xl font-bold text-slate-800">ใบเบิกสินค้า (Requisitions)</h1>
                ...
                <Card className="!p-0 flex-grow min-h-0 flex flex-col">
                    <div className="overflow-auto flex-grow">
                        <table className="min-w-full divide-y divide-slate-200">
                            <thead className="bg-slate-50 sticky top-0 z-10">
                                <tr>
                                    ...
                                    <th className="px-4 py-2.5 text-left text-sm font-medium text-slate-600 uppercase">เลขที่เอกสาร</th>
                                    <th className="px-4 py-2.5 text-left text-sm font-medium text-slate-600 uppercase">ประเภท</th>
                                    <th className="px-4 py-2.5 text-left text-sm font-medium text-slate-600 uppercase">วันที่เบิก</th>
                                    <th className="px-4 py-2.5 text-left text-sm font-medium text-slate-600 uppercase">ผู้เบิก</th>
                                    <th className="px-4 py-2.5 text-center text-sm font-medium text-slate-600 uppercase">สถานะ</th>
                            ...
                            <tbody className="bg-white divide-y divide-slate-200">
                                {paginatedRequisitions.map((item, index) => (
                                    <tr key={item.id} className="hover:bg-slate-50">
                                        ...
                                        <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-500">
                                            {item.request_date ? formatThaiDate(item.request_date) : '-'}
                                        </td>
                                        <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-500">
                        ...
                        ${item.status === RequisitionStatus.APPROVED ? 'bg-green-100 text-green-800' :
                                                    item.status === RequisitionStatus.REJECTED ? 'bg-red-100 text-red-800' :
