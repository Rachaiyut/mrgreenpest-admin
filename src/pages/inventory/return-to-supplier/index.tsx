...
import { PlusIcon, ManageIcon, EyeIcon, DocumentCheckIcon, XCircleIcon, TrashIcon } from '../../../assets/icons/Icons';
import { AddReturnToSupplierModal } from '../../../components/features/inventory/AddReturnToSupplierModal';
import { StatusBadge } from '../../../components/common/StatusBadge';
import { ReturnToSupplier, Status, Warehouse as WarehouseType, Supplier, Product } from '../../../types';
...
const ReturnToSupplierPage: React.FC<ReturnToSupplierProps> = ({ returns, onCreateReturn, onUpdateReturn, onDeleteReturn, warehouses, suppliers, products }) => {
    ...
    const filteredReturns = useMemo(() => {
        ...
        return reversedReturns.filter(r => {
            ...
            return (
                ...
                (r.referenceId && r.referenceId.toLowerCase().includes(lowercasedQuery)) ||
    ...
    const renderActions = () => {
        ...
        const actions = [
            <a key="view" href="#" onClick={(e) => { e.preventDefault(); handleViewDetails(selectedReturn); }} className="flex items-center w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-100" role="menuitem">
        ...
        if (selectedReturn.status === Status.Draft || selectedReturn.status === Status.PendingApproval) {
            actions.push(
                <a key="cancel" href="#" onClick={(e) => { e.preventDefault(); handleCancel(selectedReturn.id); }} className="flex items-center w-full text-left px-4 py-2 text-sm text-red-700 hover:bg-red-50" role="menuitem">
        ...
        <>
            <div className="p-4 sm:p-6 lg:p-8 flex flex-col h-full gap-6">
                <div className="flex-shrink-0 flex flex-wrap items-center justify-between gap-4">
                    <div>
                        <h1 className="text-3xl font-bold text-slate-800">เบิกสินค้าคืนผู้จำหน่าย</h1>
                        <p className="mt-1 text-slate-600">จัดการการเบิกสินค้าเพื่อส่งคืนผู้จำหน่าย</p>
                    </div>
                        ...
                        <table className="min-w-full divide-y divide-slate-200">
                            <thead className="bg-slate-50 sticky top-0 z-10">
                                <tr>
                                    ...
                                    <th scope="col" className="px-4 py-2.5 text-left text-sm font-medium text-slate-600 uppercase whitespace-nowrap">เลขที่เอกสาร</th>
                                    <th scope="col" className="px-4 py-2.5 text-left text-sm font-medium text-slate-600 uppercase whitespace-nowrap">อ้างอิง</th>
                                    <th scope="col" className="px-4 py-2.5 text-left text-sm font-medium text-slate-600 uppercase whitespace-nowrap">วันที่</th>
                                    <th scope="col" className="px-4 py-2.5 text-left text-sm font-medium text-slate-600 uppercase whitespace-nowrap">คืนจากคลัง</th>
                                    <th scope="col" className="px-4 py-2.5 text-left text-sm font-medium text-slate-600 uppercase whitespace-nowrap">ผู้จำหน่าย</th>
                            ...
                            <tbody className="bg-white divide-y divide-slate-200">
                                {paginatedReturns.map((r, index) => (
                                    <tr key={r.id} className="hover:bg-slate-50">
                                        ...
                                        <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-primary hover:underline cursor-pointer" onClick={() => handleViewDetails(r)}>{r.id}</td>
                                        <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-500">{r.referenceId || '-'}</td>
                                        <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-500">{formatThaiDate(r.createdAt)}</td>
                                        <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-500">{warehouseMap[r.warehouseId] || '-'}</td>
                                        <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-500">{r.supplierId ? supplierMap[r.supplierId] : '-'}</td>
                                        <td className="px-4 py-3 whitespace-nowrap text-sm"><StatusBadge status={r.status} /></td>
            ...
            <AddReturnToSupplierModal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} onCreateReturn={onCreateReturn} returns={returns} warehouses={warehouses} suppliers={suppliers} products={products} />
