...
import { GoodsReceipt as GoodsReceiptType, Status, Warehouse as WarehouseType, Supplier, Product } from '../../../types';
...
const GoodsReceipt: React.FC<GoodsReceiptProps> = ({ receipts, onCreateReceipt, onUpdateReceipt, onDeleteReceipt, warehouses, suppliers, products }) => {
    ...
    const filteredReceipts = useMemo(() => {
        ...
        return reversedReceipts.filter(receipt => {
            const supplierName = (receipt.supplierId && supplierMap[receipt.supplierId]) || '';
            const receiptDate = formatThaiDate(receipt.createdAt);

            return (
                receipt.id.toLowerCase().includes(lowercasedQuery) ||
                (receipt.referenceId && receipt.referenceId.toLowerCase().includes(lowercasedQuery)) ||
    ...
    const renderActions = () => {
        ...
        const actions = [
            <a key="view" href="#" onClick={(e) => { e.preventDefault(); handleViewDetails(selectedReceipt); }} className="flex items-center w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-100" role="menuitem">
        ...
        if (selectedReceipt.status === Status.Draft || selectedReceipt.status === Status.PendingApproval) {
            actions.push(
                <a key="cancel" href="#" onClick={(e) => { e.preventDefault(); handleCancel(selectedReceipt.id); }} className="flex items-center w-full text-left px-4 py-2 text-sm text-red-700 hover:bg-red-50" role="menuitem">
    ...
    return (
        <>
                        ...
                        <table className="min-w-full divide-y divide-slate-200">
                            <thead className="bg-slate-50 sticky top-0 z-10">
                                <tr>
                                    ...
                                    <th scope="col" className="px-4 py-2.5 text-left text-sm font-medium text-slate-600 uppercase whitespace-nowrap">เลขที่ใบรับเข้า</th>
                                    <th scope="col" className="px-4 py-2.5 text-left text-sm font-medium text-slate-600 uppercase whitespace-nowrap">เลขที่อ้างอิง</th>
                                    <th scope="col" className="px-4 py-2.5 text-left text-sm font-medium text-slate-600 uppercase whitespace-nowrap">วันที่</th>
                                    <th scope="col" className="px-4 py-2.5 text-left text-sm font-medium text-slate-600 uppercase whitespace-nowrap">คลัง</th>
                                    <th scope="col" className="px-4 py-2.5 text-left text-sm font-medium text-slate-600 uppercase whitespace-nowrap">ผู้จัดจำหน่าย</th>
                            ...
                            <tbody className="bg-white divide-y divide-slate-200">
                                {paginatedReceipts.map((receipt, index) => (
                                    <tr key={receipt.id} className="hover:bg-slate-50">
                                        ...
                                        <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-primary hover:underline cursor-pointer" onClick={() => handleViewDetails(receipt)}>{receipt.id}</td>
                                        <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-500">{receipt.referenceId || '-'}</td>
                                        <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-500">{formatThaiDate(receipt.createdAt)}</td>
                                        <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-500">{warehouseMap[receipt.warehouseId] || '-'}</td>
                                        <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-500">{receipt.supplierId ? supplierMap[receipt.supplierId] : '-'}</td>
                                        <td className="px-4 py-3 whitespace-nowrap text-sm"><StatusBadge status={receipt.status} /></td>
            ...
            <AddGoodsReceiptModal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} onCreateReceipt={onCreateReceipt} receipts={receipts} warehouses={warehouses} suppliers={suppliers} products={products} />
