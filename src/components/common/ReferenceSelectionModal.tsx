...
export const ReferenceSelectionModal: React.FC<
    ...
    return jobs.filter(
      (job) =>
        ...
        (job.id.toLowerCase().includes(searchTerm.toLowerCase()) ||
          ...
          formatThaiDate(job.created_at || job.appointment_date?.toString()).includes(searchTerm))
    );
          ...
          <table className="min-w-full divide-y divide-slate-200">
            ...
            <tbody className="bg-white divide-y divide-slate-200">
              {availableJobs.map((job) => (
                <tr
                  key={job.id}
                  className={`cursor-pointer hover:bg-slate-50 ${selectedIds.has(job.id) ? 'bg-primary/10' : ''}`}
                ...
                >
                  ...
                  <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-700">
                    {formatThaiDate(job.created_at || job.appointment_date?.toString())}
                  </td>
                </tr>
              ))}
            </tbody>
