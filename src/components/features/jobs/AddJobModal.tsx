...
import { Assessment, Status, UserRole, FieldJob, Customer, Contract, FieldJobWorkArea, User, Product } from '../../../types';
import { MOCK_WAREHOUSES, MOCK_CUSTOMERS, MOCK_PRODUCTS } from '../../../constants';
...
const JobWorkAreaForm: React.FC<{
  ...
  return (
    <div className="border border-slate-300 p-4 rounded-lg space-y-4 bg-slate-50 relative">
      {!isReadOnly && (
        <button type="button" onClick={() => onClearArea(index)} className="absolute top-2 right-2 flex items-center gap-1 text-slate-500 hover:text-slate-700 py-1 px-2 rounded-md hover:bg-slate-200 text-sm" title="ล้างค่าในพื้นที่นี้">
          <RefreshIcon className="h-4 w-4" />
          <span>ล้างค่า</span>
        </button>
      )}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <FormField label={`ชื่อพื้นที่ #${index + 1}`} htmlFor={`areaName-${index}`}>
          <Input name="name" value={area.name || ''} onChange={handleFieldChange} placeholder="เช่น ชั้น 1, โซน A" required readOnly={isReadOnly} />
        </FormField>
        <FormField label="แพ็กเกจ/ประเภทบริการ" htmlFor={`servicePackage-${index}`}>
          <Input name="servicePackage" value={area.servicePackage || ''} onChange={handleFieldChange} required readOnly={isReadOnly} />
        </FormField>
      </div>
    </div>
  );
...
export const AddJobModal: React.FC<AddJobModalProps> = ({ isOpen, onClose, assessments, contracts, onCreateJob, jobs, users, products, initialContractId, initialWorkDateIso }) => {
  ...
  const vehicleWarehouses = useMemo(() => MOCK_WAREHOUSES.filter(w => w.type === 'รถ'), []);
  const productMap = useMemo(() => new Map(products.map(p => [p.id, p])), [products]);

  const availableAssessments = useMemo(() => {
    const baseAssessments = assessments.filter(a => a.status === Status.Draft || a.status === Status.Completed);
  ...
  const availableContracts = useMemo(() => {
    ...
    return contracts.filter(c => c.customerId === selectedCustomerId && c.status === Status.InProgress);
  ...
  const selectedAssessment = isAssessment ? assessments.find(a => a.id === selectedReference.replace('asm-', '')) : null;
  const selectedContract = isContract ? contracts.find(c => c.id === selectedReference.replace('con-', '')) : null;
  ...
  useEffect(() => {
    if (selectedAssessment) {
      ...
      if (workAreas.length === 0) {
        setWorkAreas([{ id: `area-${Date.now()}`, name: 'พื้นที่ 1', servicePackage: '' }]);
      }
    } else {
      // Nothing selected
      setWorkAreas([]);
  ...
  useEffect(() => {
    ...
    if (selectedAssessment) {
      ...
      selectedAssessment.workAreas.forEach((area, index) => {
        ...
        if (area.buildingType) details += `- ประเภทสิ่งปลูกสร้าง: ${area.buildingType}\n`;
        if (area.areaSize) details += `- ขนาดพื้นที่: ${area.areaSize} ตร.ม.\n`;
        if (area.serviceSystem) details += `- ระบบที่ใช้: ${area.serviceSystem}\n`;
        if (area.packageId) {
  ...
  useEffect(() => {
    ...
    if (conflictingJob) {
      setTimeConflictError(`เวลานี้ทับซ้อนกับงานของ ${conflictingJob.customerName} (${new Date(conflictingJob.startTime).toTimeString().substring(0, 5)} - ${new Date(conflictingJob.endTime).toTimeString().substring(0, 5)})`);
    } else {
      setTimeConflictError(null);
    }
  ...
  const createJobObject = (status: Status): Omit<FieldJob, 'id'> | null => {
    if (!selectedCustomer || !workDate || !startTime || !endTime || workAreas.length === 0 || !leadTechnicianId) return null;
    ...
    const assignedTechnicians = users.filter(u => uniqueTechnicianIds.includes(u.id));
  ...
  const handleAreaChange = (index: number, updatedArea: Partial<FieldJobWorkArea>) => {
  ...
  const handleClearArea = (index: number) => {
    setWorkAreas(prev => {
      ...
      if (areaToClear) {
        newAreas[index] = { id: areaToClear.id, name: areaToClear.name, servicePackage: '' };
      }
      return newAreas;
    });
  };

  const additionalTechnicians = technicians.filter(tech => tech.id !== leadTechnicianId);
    ...
    <Modal
      ...
      footer={
        <div className="flex gap-2">
          <Button type="button" onClick={onClose} variant="secondary">ยกเลิก</Button>
          <Button type="submit" form="add-job-form" variant="primary" disabled={!!timeConflictError}>บันทึก</Button>
        </div>
      }
    >
      <form id="add-job-form" onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border-b pb-4 mb-4">
          <FormField label="วันที่ปฏิบัติงาน" htmlFor="work-date">
            <Input id="work-date" type="date" value={workDate} onChange={e => setWorkDate(e.target.value)} required />
          </FormField>
        ...
        {bookedSlots.length > 0 && (
          <div className="p-3 bg-amber-50 border border-amber-200 rounded-md text-sm mb-4">
            <p className="font-semibold text-amber-800">ช่วงเวลาที่ไม่ว่างสำหรับรถคันนี้ในวันที่เลือก:</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <FormField label="เลือกลูกค้า" htmlFor="customer-select">
            <Select id="customer-select" value={selectedCustomerId} onChange={e => handleCustomerChange(e.target.value)} required>
          ...
          <FormField label="หรือ อ้างอิง (ใบประเมิน/สัญญา)" htmlFor="reference-select">
            <Select id="reference-select" value={selectedReference} onChange={e => handleReferenceChange(e.target.value)} disabled={!selectedCustomerId}>
              <option value="">-- ไม่เลือกรายการอ้างอิง --</option>
              {availableAssessments.length > 0 && (
                <optgroup label="ใบประเมิน">
                  {availableAssessments.map(a => (
                    <option key={`asm-${a.id}`} value={`asm-${a.id}`}>{a.id} - {a.workAreas[0] ? a.workAreas[0].serviceType.join(', ') : ''}</option>
                  ))}
                </optgroup>
              )}
              {availableContracts.length > 0 && (
                <optgroup label="สัญญา">
                  {availableContracts.map(c => (
                    <option key={`con-${c.id}`} value={`con-${c.id}`}>{c.id} - {c.servicePackage}</option>
                  ))}
                </optgroup>
              )}
            </Select>
        ...
        {workAreas.length > 0 && (
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-slate-800 border-b pb-2">รายละเอียดพื้นที่บริการ</h3>
            {selectedReference ? (
              <div className="p-3 bg-blue-50 border border-blue-200 rounded-md text-sm text-blue-700">
                ข้อมูลพื้นที่ถูกดึงมาจาก {isAssessment ? 'ใบประเมิน' : 'สัญญา'} เลขที่: <strong>{selectedAssessment?.id || selectedContract?.id}</strong>
              </div>
            ) : selectedCustomerId && (
              <FormField label="จำนวนพื้นที่ที่ต้องการเข้าบริการ" htmlFor="numberOfAreas">
        ...
        <FormField label="เลือกรถที่ปฏิบัติงาน" htmlFor="vehicle-select">
          <Select id="vehicle-select" value={selectedVehicleId} onChange={e => setSelectedVehicleId(e.target.value)} required>
        ...
        {timeConflictError && <p className="text-sm text-red-600 -mt-2">{timeConflictError}</p>}

        <FormField label="หัวหน้าช่าง" htmlFor="lead-technician-select">
          <Select id="lead-technician-select" value={leadTechnicianId} onChange={e => handleLeadTechnicianChange(e.target.value)} required>
        ...
        <FormField label="ช่างเทคนิคเพิ่มเติม (ถ้ามี)">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2 p-2 border rounded-md max-h-40 overflow-y-auto">
            {additionalTechnicians.map(tech => (
              <label key={tech.id} className="flex items-center space-x-2 p-2 rounded-md hover:bg-slate-100 cursor-pointer">
                <input
                  type="checkbox"
