...
export const EditUserModal: React.FC<EditUserModalProps> = ({ isOpen, onClose, user, onUpdateUser }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (user) {
            if (!formData.name || !formData.phone || !formData.role || !formData.nickname) {
        ...
        <Modal
            ...
            footer={
                <div className="flex gap-2">
                    ...
                    <Button variant="primary" type="submit" form="edit-user-form" className="py-2 px-4 rounded-lg bg-primary hover:bg-primary/90 text-white font-semibold shadow-sm">
                                ...
                                <div className="space-y-1 text-center">
                                    ...
                                    <div className="flex text-sm text-slate-600 justify-center pt-2">
                                        <label htmlFor="file-upload-edit" className="relative cursor-pointer bg-white rounded-md font-medium text-primary hover:text-primary-dark focus-within:outline-none">
                                            ...
                                            <input id="file-upload-edit" name="file-upload" type="file" className="sr-only" accept="image/png, image/jpeg" onChange={handleImageChange} />
                                        </label>
                                    </div>
                    ...
                    <div className="md:col-span-2 space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <FormField label="เลขบัตรประชาชน (Username)" htmlFor="nationalId">
                                <Input id="nationalId" name="nationalId" type="text" value={formData.nationalId || ''} readOnly className="bg-slate-100"/>
                            </FormField>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <FormField label="ชื่อเล่น" htmlFor="nickname">
                                <Input id="nickname" name="nickname" type="text" value={formData.nickname || ''} onChange={handleChange} required />
                            </FormField>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <FormField label="เลือกบทบาท" htmlFor="role">
                             ...
                             <FormField label="จำกัดการเบิก (บาท)" htmlFor="creditLimit">
                                <Input id="creditLimit" name="creditLimit" type="number" placeholder="0.00" step="0.01" value={formData.creditLimit ?? ''} onChange={handleChange} />
                            </FormField>
                        </div>
                    </div>
                </div>
