    ...
    >
        <div className="flex items-start space-x-6">
            ...
            <div className="space-y-4 text-sm flex-grow">
                ...
                <DetailsList cols={1}>
                    <DetailsItem label="ชื่อ-นามสกุล" valueClassName="font-semibold">{user.name}</DetailsItem>
                    <DetailsItem label="ชื่อเล่น" valueClassName="font-semibold">{user.nickname}</DetailsItem>
                    <DetailsItem label="เลขบัตรประชาชน (Username)">{user.nationalId}</DetailsItem>
                    ...
                    <DetailsItem label="จำกัดการเบิก (บาท)" valueClassName="font-semibold">{typeof user.creditLimit === 'number' ? `฿${user.creditLimit.toLocaleString('th-TH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : '-'}</DetailsItem>
                </DetailsList>
            </div>
        </div>
    </Modal>
