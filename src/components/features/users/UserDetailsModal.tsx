              ...
              <p className="text-slate-500 text-sm">
                {(() => {
                  ...
                  if (typeof role === 'string') roleNameRaw = role;
                    ...
                    const r = role as any;
                    roleNameRaw = typeof r.name === 'string' ? r.name : JSON.stringify(r.name);
                  }
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-xs text-slate-500 mb-1 block">ชื่อเล่น</label>
                  <div className="text-slate-800 font-medium text-lg">{user.nick_name || '-'}</div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs text-slate-500 mb-1 block">เลขบัตรประชาชน (Username)</label>
                    <div className="bg-slate-50 border border-slate-100 rounded-lg px-3 py-2 flex items-center justify-between">
                      <span className="font-mono text-slate-700">{user.citizen_id || '-'}</span>
                      {user.citizen_id && <CopyButton text={user.citizen_id} />}
                    </div>
                  </div>
                  <div>
                    <label className="text-xs text-slate-500 mb-1 block">เบอร์โทรศัพท์</label>
                ...
                <div>
                  <label className="text-xs text-slate-500 mb-1 block">อีเมล</label>
                  <div className="flex items-center gap-2">
            ...
            {typeof user.creditLimit === 'number' && (
              <div className="pt-4 border-t border-slate-100">
                <div className="bg-white rounded-xl flex items-center justify-between">
                  <div>
                    <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">วงเงินจำกัดการเบิก</p>
