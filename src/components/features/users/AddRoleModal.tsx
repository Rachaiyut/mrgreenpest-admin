...
export const AddRoleModal: React.FC<AddRoleModalProps> = ({ isOpen, onClose }) => {
    ...
    const handlePermissionChange = (item: string, action: string, checked: boolean) => {
        setPermissions(prev => ({
            ...prev,
            [item]: {
                ...prev[item],
    ...
    return (
        <Modal 
            ...
            footer={
                <div className="flex gap-2">
                    ...
                    <button type="submit" form="add-role-form" className="py-2 px-4 rounded-lg bg-primary hover:bg-primary/90 text-white font-semibold shadow-sm">
                        บันทึก
                    </button>
                </div>
            }
        >
            <form id="add-role-form" onSubmit={handleSubmit} className="space-y-6">
                <div>
                    <label htmlFor="role-name" className="block text-sm font-medium text-slate-700 mb-1">ชื่อบทบาท<span className="text-red-500">*</span></label>
                    <Input id="role-name" type="text" required placeholder="เช่น ผู้จัดการฝ่ายขาย" />
                </div>
                        ...
                        <thead className="bg-slate-50">
                            <tr>
                                ...
                                {PERMISSION_ACTIONS.map(action => {
                                    const enabledItemsForAction = allPermissionItems.filter(item => !isActionDisabled(item, action));
                                    const areAllChecked = enabledItemsForAction.length > 0 && enabledItemsForAction.every(item => permissions[item]?.[action]);
                                    return (
                                        <th key={action} scope="col" className="px-4 py-3 text-center font-medium text-slate-600 uppercase tracking-wider">
                                            <label className="flex flex-col items-center justify-center gap-1">
                                                ...
                                                <input
                                                    ...
                                                    onChange={(e) => handleSelectAllForColumn(action, e.target.checked)}
                                                />
                                            </label>
                                        </th>
                                    );
                                })}
                                <th scope="col" className="px-4 py-3 text-center font-medium text-slate-600 uppercase tracking-wider">
                                     <label className="flex flex-col items-center justify-center gap-1">
                                        <span>ทั้งหมด</span>
                                     </label>
                                </th>
                            ...
                            {PERMISSION_GROUPS.map(group => (
                                <React.Fragment key={group.groupName}>
                                    ...
                                    {group.items.map(item => {
                                        const enabledActionsForRow = PERMISSION_ACTIONS.filter(action => !isActionDisabled(item, action));
                                        const areAllForRowChecked = enabledActionsForRow.length > 0 && enabledActionsForRow.every(action => permissions[item]?.[action]);
                                            ...
                                            <tr key={item} className="hover:bg-slate-50">
                                                        ...
                                                        <td key={action} className="px-4 py-3 text-center">
                                                            {!isDisabled ? (
                                                                <input
                                                                    ...
                                                                    onChange={(e) => handlePermissionChange(item, action, e.target.checked)}
                                                                />
                                                            ) : null}
                                                        </td>
                                                    );
                                                ...
                                                <td className="px-4 py-3 text-center">
                                                     <input
                                                        ...
                                                        onChange={(e) => handleSelectAllForRow(item, e.target.checked)}
                                                    />
                                                </td>
                                            </tr>
                                        );
