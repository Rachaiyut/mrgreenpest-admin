...
interface SetWithdrawalLimitModalProps {
  ...
  onSave: (warehouseId: string, limits: { [productId: string]: number }) => void;
  products: Product[];
}

export const SetWithdrawalLimitModal: React.FC<SetWithdrawalLimitModalProps> = ({ isOpen, onClose, warehouse, onSave, products }) => {
  ...
  const availableProducts = useMemo(() => {
    ...
    return products.filter(p => p.type === 'สินค้า' && p.warehouse === 'คลังหลัก');
  }, [products]);
  ...
  const handleLimitChange = (product: Product, value: string) => {
    ...
    setLimits(prev => ({ ...prev, [product.id]: value === '' ? '' : numValue }));

    if (value !== '' && (isNaN(numValue) || numValue < 0)) {
        setErrors(prev => ({ ...prev, [product.id]: 'กรุณาระบุตัวเลขที่ถูกต้อง' }));
    } else {
        setErrors(prev => {
          ...
          <table className="min-w-full divide-y divide-slate-200">
            <thead className="bg-slate-50 sticky top-0">
              <tr>
                ...
                <th className="px-4 py-3 text-center text-xs font-medium text-slate-500 uppercase">คงคลัง (คลังหลัก)</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">จำกัดการเบิก (หน่วย)</th>
              </tr>
            </thead>
                ...
                <tr key={product.id}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-col">
                        ...
                        />
                         {errors[product.id] && <p className="text-xs text-red-600 mt-1">{errors[product.id]}</p>}
                    </div>
                  </td>
                </tr>
              ))}
