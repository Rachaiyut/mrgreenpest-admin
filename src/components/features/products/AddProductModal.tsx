  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const newProduct: Partial<IProduct> = {
        ...
        code: '', // Let backend handle code generation if possible, or leave empty
        min_stock: Number(data['low-stock-threshold']) || 0,
        unit: { name: data.unit as string },
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormField label="หน่วย" htmlFor="unit">
                <Select name="unit" id="unit" required>
                  ...
                  {units.map((unit) => (
                    <option key={unit.id} value={unit.name}> {/* หรือ unit.id ตามที่ Backend รับ */}
                      {unit.name}
                    </option>
                  ))}
                </Select>
