...
export const AddProductModal: React.FC<AddProductModalProps> = ({ isOpen, onClose, onCreateProduct, products, categories }) => {
    const [productType, setProductType] = useState<'สินค้า' | 'บริการ'>('สินค้า');
    const [imagePreview, setImagePreview] = useState<string | null>(null);

    const availableCategories = useMemo(() => categories.filter(c => c.type === productType), [categories, productType]);
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const newProduct: Omit<Product, 'id'> = {
            ...
            costPrice: productType === 'สินค้า' && data.costPrice ? parseFloat(data.costPrice as string) : undefined,
            fdaRegNo: productType === 'สินค้า' ? data.fdaRegNo as string | undefined : undefined,
            stock: productType === 'สินค้า' ? 0 : 9999, // default value
            lowStockThreshold: productType === 'สินค้า' ? parseInt(data['low-stock-threshold'] as string, 10) : 0,
        ...
        <Modal
            ...
            footer={
                <div className="flex gap-2">
                    ...
                    <button type="submit" form="add-product-form" className="py-2 px-4 rounded-lg bg-primary hover:bg-primary/90 text-white font-semibold shadow-sm">
                        บันทึก
                    </button>
                </div>
            }
                                ...
                                <div className="space-y-1 text-center">
                                    ...
                                    <div className="flex text-sm text-slate-600 justify-center">
                                        <label htmlFor="file-upload" className="relative cursor-pointer bg-white rounded-md font-medium text-primary hover:text-primary-dark focus-within:outline-none">
                                            ...
                                            <input id="file-upload" name="file-upload" type="file" className="sr-only" accept="image/png, image/jpeg" onChange={handleImageChange} required={!imagePreview} />
                                        </label>
                                    </div>
                    ...
                    <div className="md:col-span-2 space-y-4">
                        <FormField label="ประเภท">
                            <div className="flex rounded-lg bg-slate-100 p-1 w-full">
                                <label className="relative flex-1 cursor-pointer">
                                    <input type="radio" name="productType" value="สินค้า" className="sr-only peer" checked={productType === 'สินค้า'} onChange={() => setProductType('สินค้า')} />
                                ...
                                <label className="relative flex-1 cursor-pointer">
                                    <input type="radio" name="productType" value="บริการ" className="sr-only peer" checked={productType === 'บริการ'} onChange={() => setProductType('บริการ')} />
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <FormField label="รหัสสินค้า/บริการ" htmlFor="product-id">
                                <Input id="product-id" type="text" value={generatedId} readOnly className="bg-slate-100" />
                            </FormField>
                            <FormField label="รหัสบาร์โค้ด" htmlFor="barcode">
                                <Input name="barcode" id="barcode" type="text" disabled={productType === 'บริการ'} />
                            </FormField>
                        </div>
                    ...
                    <>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <FormField label="หน่วย" htmlFor="unit">
                                <Input name="unit" id="unit" type="text" required placeholder="เช่น ขวด, ชิ้น" />
                            </FormField>
