...
export const CustomerSelectionModal: React.FC<CustomerSelectionModalProps> = ({ isOpen, onClose, onConfirm, customers, initialSelectedIds }) => {
                ...
                <table className="min-w-full divide-y divide-slate-200">
                    <thead className="bg-slate-50 sticky top-0">
                        <tr>
                            ...
                            <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">รหัสลูกค้า</th>
                            <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">ชื่อลูกค้า</th>
                            <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">ผู้ติดต่อ</th>
                            <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">เบอร์โทรศัพท์</th>
                            <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">ประเภท</th>
                        </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-slate-200">
                        {filteredCustomers.map(customer => (
                            <tr key={customer.id} className={`cursor-pointer hover:bg-slate-50 ${selectedIds.has(customer.id) ? 'bg-primary/10' : ''}`} onClick={() => handleToggleSelection(customer.id)}>
