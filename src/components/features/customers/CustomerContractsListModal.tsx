...
export const CustomerContractsListModal: React.FC<CustomerContractsListModalProps> = ({ isOpen, onClose, customer, contracts, quotations, onCreateContract, onCreateJob }) => {
    ...
    const handleAutoSchedule = (contract: Contract) => {
        ...
        if (!confirm(`ยืนยันการสร้างตารางงานอัตโนมัติสำหรับสัญญา ${contract.id}?`)) return;
            ...
            <Modal
                ...
                footer={
                    <div className="flex w-full justify-between items-center">
                        <div></div>
                        <Button type="button" onClick={onClose} className="py-2 px-4 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold border border-slate-300" variant="outline">ปิด</Button>
                    </div>
                }
            >
                    ...
                    <div className="overflow-x-auto border border-slate-200 rounded-lg max-h-[60vh] overflow-y-auto">
                        <table className="min-w-full divide-y divide-slate-200">
                            <thead className="bg-slate-50 sticky top-0">
                                <tr>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">เลขที่สัญญา</th>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">ที่อยู่</th>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">วันเริ่มต้น</th>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">วันสิ้นสุด</th>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">สถานะ</th>
                                    <th className="px-4 py-3 text-center text-xs font-medium text-slate-500 uppercase">จัดการ</th>
                                </tr>
                            </thead>
                            <tbody className="bg-white divide-y divide-slate-200">
                                {customerContracts.length > 0 ? (
                                    customerContracts.map(contract => (
                                        <tr key={contract.id}>
                                            <td className="px-4 py-3 text-sm font-medium text-slate-800">{contract.id}</td>
                                            <td className="px-4 py-3 text-sm text-slate-600 truncate max-w-xs">{contract.address}</td>
                                            <td className="px-4 py-3 text-sm text-slate-600">{formatThaiDate(contract.startDate)}</td>
                                            <td className="px-4 py-3 text-sm text-slate-600">{formatThaiDate(contract.endDate)}</td>
                                            <td className="px-4 py-3 text-sm"><StatusBadge status={contract.status} /></td>
