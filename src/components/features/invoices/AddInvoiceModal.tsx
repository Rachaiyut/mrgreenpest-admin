...
export const InvoiceForm: FC<InvoiceFormProps> = ({
    ...
    useEffect(() => {
        if (selectedContractId && selectedInstallmentTerm) {
            ...
            if (contract) {
                // In a real scenario, we might need an API to get the next sequence
                // For now, let's just show the format pattern
                // Format: ContractCode-Term-Sequence (e.g. 68100-1-1)
                setAutoGeneratedCode(`${contract.code || 'CT'}-${selectedInstallmentTerm}-X`);
            }
        } else {
    ...
    // Installments from selected contract
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const selectedInstallment = contractInstallments.find(i => i.term === selectedInstallmentTerm);
            
            const payload = {
                // If manual code was entered (and it differs from auto-gen pattern), use it.
                // Otherwise, let backend generate.
                code: (invoiceCode && invoiceCode !== autoGeneratedCode) ? invoiceCode : undefined,
                             ...
                             <div className="flex items-center gap-2">
                                ...
                                <Select
                                    ...
                                    className={`w-40 font-medium border-0 ring-1 ring-inset py-1.5 h-9 text-sm ${
                                        status === InvoiceStatus.PAID ? 'text-green-700 bg-green-50 ring-green-600/20' :
