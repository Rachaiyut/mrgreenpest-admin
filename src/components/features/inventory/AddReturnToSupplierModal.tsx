...
import { ReturnToSupplier, ReturnToSupplierItem, Status, Warehouse as WarehouseType, Supplier, Product } from '../../../types';
...
export const AddReturnToSupplierModal: FC<AddReturnToSupplierModalProps> = ({ isOpen, onClose, onCreateReturn, returns, warehouses, suppliers, products }) => {
    ...
    const createReturnObject = (status: Status, formData: FormData): Omit<ReturnToSupplier, 'id'> => {
            ...
            <Modal
                ...
                footer={
                    <div className="flex gap-2">
                        ...
                        <Button variant="primary" type="submit" form="add-rts-form">ส่งเพื่ออนุมัติ</Button>
                    </div>
                }
            >
                <form ref={formRef} id="add-rts-form" onSubmit={handleSubmit} className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <FormField label="วันที่ทำรายการ" htmlFor="return-date">
                            <Input name="return-date" id="return-date" type="date" defaultValue={new Date().toISOString().substring(0, 10)} required />
                        </FormField>
                    ...
                    <FormField label="อ้างอิงเอกสารรับเข้า (GR) / อื่นๆ" htmlFor="reference-id">
                        <Input name="reference-id" id="reference-id" type="text" placeholder="เช่น GR670001" />
                    </FormField>
                    ...
                    <div>
                        <div className="flex justify-between items-center mb-2">
                            <h4 className="text-base font-semibold text-slate-800">รายการสินค้าที่จะคืน</h4>
                            <Button
                                variant="primary"
                                type="button"
                            ...
                            <table className="min-w-full text-sm">
                                <thead className="bg-slate-50">
                                    <tr>
                                        ...
                                        <th className="p-2 text-left font-medium text-slate-600">สาเหตุการคืน</th>
                                ...
                                <tbody>
                                    {items.length > 0 ? items.map((item, index) => {
                                        ...
                                        return (
                                            <tr key={item.id} className="border-b border-slate-200 last:border-b-0">
                                                ...
                                                <td className="px-2 py-3 align-middle">
                                                    <Input type="number" value={item.quantity} onChange={e => handleItemChange(item.id, 'quantity', parseInt(e.target.value) || 0)} className="w-24 h-10" min="1" required />
                                                </td>
                                                <td className="px-2 py-3 align-middle">
                                                    <Input type="text" value={item.reason} onChange={e => handleItemChange(item.id, 'reason', e.target.value)} className="w-full h-10" placeholder="ระบุสาเหตุ" />
                                                </td>
                                        ...
                                        <tr>
                                            <td colSpan={7} className="px-2 py-6 text-center text-slate-500">ยังไม่มีรายการสินค้า</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
