...
export const TransferDetailsModal: React.FC<TransferDetailsModalProps> = ({
  ...
  const totalQuantity = transfer.items?.reduce((sum, item) => sum + Number(item.qty || item.quantity || 0), 0) || 0;
        ...
        <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
             ...
             <div className="bg-amber-50 p-3 rounded border border-amber-100">
               ...
               <p className="font-semibold text-amber-900 text-lg">
                 {warehouseMap[transfer.from_warehouse_id] || (transfer as any).from_warehouse?.name || '-'}
               </p>
             </div>
             ...
             <div className="bg-blue-50 p-3 rounded border border-blue-100">
               ...
               <p className="font-semibold text-blue-900 text-lg">
                 {warehouseMap[transfer.to_warehouse_id] || (transfer as any).to_warehouse?.name || '-'}
               </p>
             </div>
          </div>
          ...
          {transfer.remark && (
            <div className="mt-4 pt-3 border-t border-slate-100">
              <p className="text-xs text-slate-500 mb-1">หมายเหตุ / เหตุผลการโอนย้าย</p>
              ...
              <tbody className="bg-white divide-y divide-slate-200">
                {transfer.items && transfer.items.length > 0 ? (
                  transfer.items.map((item, index) => {
                    const productId = item.product_id || (item as any).productId;
                    const product = productMap.get(productId);
                    const qty = Number(item.qty || item.quantity || 0);
                    return (
                      <tr key={`${transfer.id}-${productId}-${index}`} className="hover:bg-slate-50 transition-colors">
                  ...
                  <tr>
                    <td colSpan={5} className="text-center py-12 text-slate-500 bg-slate-50/50">
                       <div className="flex flex-col items-center justify-center">
                        ...
                        <p className="font-medium">ไม่มีรายการสินค้าในใบโอนย้ายนี้</p>
                      </div>
                    </td>
                  </tr>
                )}
