...
export const AddTransferModal: React.FC<AddTransferModalProps> = ({ isOpen, onClose, onCreateTransfer, transfers, warehouses, products, stockMap }) => {
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        if (!isFormValid) {
            alert('กรุณากรอกข้อมูลให้ครบถ้วน: ต้องมีคลังต้นทาง, คลังปลายทาง, เหตุผล และมีสินค้าอย่างน้อย 1 รายการที่จำนวนมากกว่า 0');
            return;
        }
            ...
            <Modal 
                ...
                footer={
                    <div className="flex gap-2">
                        ...
                        <Button variant="primary" type="submit" form="add-transfer-form">บันทึก</Button>
                    </div>
                }
            >
                <form id="add-transfer-form" onSubmit={handleSubmit} className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        <FormField label="เลขที่เอกสารโอนย้าย" htmlFor="transfer-id">
                            <Input id="transfer-id" type="text" value={generatedId} readOnly className="bg-slate-100" />
                        </FormField>
                        <FormField label="วันที่โอนย้าย" htmlFor="transfer-date">
                            <Input id="transfer-date" name="createdAt" type="date" defaultValue={new Date().toISOString().substring(0, 10)} required />
                        </FormField>
                    </div>
                    ...
                    <div>
                         <div className="flex justify-between items-center mb-2">
                            ...
                            <Button
                                ...
                                title={!fromWarehouseId ? 'กรุณาเลือกคลังต้นทางก่อน' : 'เพิ่มสินค้า'}
                                className="text-sm"
                            >
                            ...
                            <table className="min-w-full text-sm">
                                <thead className="bg-slate-50">
                                    <tr>
                                        ...
                                        <th className="p-2 text-center font-medium text-slate-600">จำนวนคงคลัง</th>
                                    ...
                                    {items.length > 0 ? items.map((item, index) => {
                                        ...
                                        return (
                                            <tr key={item.id} className="border-b border-slate-200 last:border-b-0">
                                                ...
                                                <td className="p-2 align-middle text-slate-600">{product?.id || '-'}</td>
                                                <td className="p-2 align-middle font-medium text-slate-800">{product?.name || 'N/A'}</td>
                                                <td className="p-2 align-middle text-center text-slate-600">{currentStock}</td>
                                                <td className="p-2 align-middle">
                                                    <Input 
                                                        ...
                                                        onChange={e => {
                                                            ...
                                                            const validatedQuantity = Math.min(newQuantity, currentStock);
                                                            handleItemChange(item.id, 'quantity', validatedQuantity);
                                                        }}
                                                        className="w-24 h-10" 
                                                        min="1" 
                                                        max={currentStock}
