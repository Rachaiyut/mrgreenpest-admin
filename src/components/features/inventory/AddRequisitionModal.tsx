...
export const AddRequisitionModal: React.FC<AddRequisitionModalProps> = ({
  ...
  useEffect(() => {
    if (refType === 'JOB') {
      JobApi.getAll({ status: JobMainStatus.COMPLETE })
        .then((res) => {
          // Deduplicate jobs to prevent key collisions
          const uniqueJobs = Array.from(
            new Map(res.data.map((j) => [j.id, j])).values()
          );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload: CreateRequisitionDto = {
        ...
        vehicle_id:
          type === ReqTypeEnum.ITEM && vehicleId ? vehicleId : undefined,
        description:
          description +
          (refType === 'JOB' && refJobId ? `\nRef Job: ${refJobId}` : ''),
        items:
        ...
        <form className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <FormField label="ประเภท (Type)">
              <Select
                value={type}
                onChange={(e) => setType(e.target.value as ReqTypeEnum)}
              >
                <option value={ReqTypeEnum.ITEM}>เบิกสินค้า (Item)</option>
                <option value={ReqTypeEnum.EXPENSE}>
                      ...
                      <tr key={item.id}>
                        ...
                        <td className="p-2 border">
                          ...
                          <div className="text-xs text-gray-400">
                            คงเหลือ: {stockMap.get(item.productId) || 0}
                          </div>
                        </td>
                        <td className="p-2 border">
                          <Input
