...
import { Withdrawal, Warehouse as WarehouseType, Product } from '../../../types';
...
export const WithdrawalDetailsModal: React.FC<WithdrawalDetailsModalProps> = ({ isOpen, onClose, withdrawal, warehouses, products }) => {
    ...
    <Modal 
        ...
        footer={
             <div className="flex w-full justify-between items-center">
                <p className="text-lg font-semibold text-slate-800">ยอดรวมทั้งหมด: <span className="text-primary">฿{grandTotal.toLocaleString('th-TH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span></p>
                <Button variant="primary" type="button" onClick={onClose}>ปิด</Button>
            </div>
        }
    >
      <div className="space-y-6">
        <div>
            ...
            <dl className="grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-6 text-sm">
                <div>
                    ...
                    <dd className="mt-1 text-slate-900 font-semibold">{withdrawal.id}</dd>
                </div>
                <div>
                    <dt className="font-medium text-slate-500">สถานะ</dt>
                    <dd className="mt-1"><StatusBadge status={withdrawal.status} /></dd>
                </div>
                <div>
                    <dt className="font-medium text-slate-500">วันที่สร้าง</dt>
                    <dd className="mt-1 text-slate-900">{formatThaiDate(withdrawal.createdAt)}</dd>
                </div>
                <div>
                 ...
                 <div>
                    ...
                    <dd className="mt-1 text-slate-900">{toWarehouse?.name || '-'}</dd>
                </div>
                 <div className="md:col-span-3">
                    <dt className="font-medium text-slate-500">เลขที่อ้างอิง</dt>
                    <dd className="mt-1 flex flex-wrap gap-2">
                        {withdrawal.referenceIds && withdrawal.referenceIds.length > 0 ? (
                            withdrawal.referenceIds.map(id => (
                                <span key={id} className="px-2 py-1 bg-slate-200 text-slate-800 text-xs font-medium rounded-md">{id}</span>
                            ))
                        ) : (
                ...
                {withdrawal.approvedBy && (
                 ...
                 {withdrawal.remarks && (
                    <div className="md:col-span-3">
                        ...
                        <dd className="mt-1 text-slate-900 bg-slate-50 p-2 rounded-md">{withdrawal.remarks}</dd>
                    </div>
                )}
            </dl>
        </div>
        ...
        <div>
            <h4 className="text-base font-semibold text-slate-800 mb-3">รายการสินค้า/อุปกรณ์ที่เบิก</h4>
                ...
                <table className="min-w-full divide-y divide-slate-200">
                    <thead className="bg-slate-50 sticky top-0">
                        <tr>
                            ...
                            <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase">สินค้า</th>
                            <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase">จำนวน</th>
                            <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase">หน่วย</th>
                            <th className="px-6 py-3 text-right text-xs font-medium text-slate-500 uppercase">มูลค่า (บาท)</th>
                        </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-slate-200">
                        {withdrawal.items.length > 0 ? (
                            ...
                            <tr>
                                <td colSpan={5} className="px-6 py-4 text-center text-sm text-slate-500">ไม่มีรายการสินค้า</td>
                            </tr>
                        )}
                    </tbody>
                    <tfoot className="bg-slate-50">
                        <tr>
                            <td colSpan={4} className="px-6 py-3 text-right text-sm font-medium text-slate-900">รวมค่าสินค้า/อุปกรณ์</td>
                            <td className="px-6 py-3 text-right text-sm font-medium text-slate-900">
                                {totalGoodsAmount.toLocaleString('th-TH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            </td>
                        </tr>
                    </tfoot>
                </table>
        ...
        {withdrawal.expenses && withdrawal.expenses.length > 0 && (
            <div>
                <h4 className="text-base font-semibold text-slate-800 mb-3">รายการค่าใช้จ่ายเพิ่มเติม</h4>
                <div className="overflow-hidden border border-slate-200 rounded-lg max-h-60 overflow-y-auto">
                    <table className="min-w-full divide-y divide-slate-200">
                        <thead className="bg-slate-50 sticky top-0">
                            <tr>
                                <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase">ลำดับ</th>
                                <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase">รายการ</th>
                                <th className="px-6 py-3 text-right text-xs font-medium text-slate-500 uppercase">จำนวนเงิน (บาท)</th>
                            </tr>
                        </thead>
                        <tbody className="bg-white divide-y divide-slate-200">
                            {withdrawal.expenses.map((exp, index) => (
                                <tr key={index}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-900 text-right">
                                        {exp.amount.toLocaleString('th-TH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                    </td>
                                </tr>
                            ))}
                        </tbody>
                         <tfoot className="bg-slate-50">
                            <tr>
                                <td colSpan={2} className="px-6 py-3 text-right text-sm font-medium text-slate-900">รวมค่าใช้จ่ายเพิ่มเติม</td>
                                <td className="px-6 py-3 text-right text-sm font-medium text-slate-900">
