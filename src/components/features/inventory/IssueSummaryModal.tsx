...
export const IssueSummaryModal: React.FC<IssueSummaryModalProps> = ({
  ...
  const formId = isEditMode ? 'edit-stock-issue-summary-form' : 'add-stock-issue-summary-form';
  ...
  return (
    <>
      <Modal
        isOpen={isOpen}
        onClose={onClose}
        title={isEditMode ? 'แก้ไขสรุปเบิกสินค้า/อุปกรณ์' : 'สร้างสรุปเบิกสินค้า/อุปกรณ์'}
        size="5xl"
        footer={
            ...
            <div className="flex items-center gap-4 text-sm text-slate-500">
              ...
              {(isOverLimit || isAnyItemOverLimit) && (
                <span className="flex items-center gap-1 text-amber-600 bg-amber-50 px-3 py-1 rounded-md border border-amber-200 text-sm font-medium">
                  ⚠️ ยอดรวมหรือจำนวนสินค้าเกินที่กำหนด{isEditMode ? ' (ควรตั้งเป็น PENDING เพื่อรออนุมัติ)' : ' (ต้องได้รับการอนุมัติ)'}
                </span>
              )}
            </div>
      ...
      >
        <form ref={goodsFormRef} id={formId} onSubmit={handleSubmit}>
          <div className="flex flex-col gap-6">
                            ...
                            <div className="grid grid-cols-12 gap-4 items-center w-full">
                              <div className="flex flex-col justify-center col-span-4">
                                ...
                                <div className="flex items-center gap-2 mt-1.5">
                                  <span className="font-mono text-[11px] font-semibold bg-slate-100 text-slate-500 px-2 py-0.5 rounded border border-slate-200">
                                    {(product as any)?.code || product?.id?.substring(0, 8) || item.product_id.substring(0, 8)}
                                  </span>
                                </div>
                              </div>
                              ...
                              <div className="flex flex-col items-center justify-center col-span-2">
                                ...
                                <span className={`text-base font-bold ${available === 0 ? 'text-red-500' : 'text-slate-700'}`}>
                                  ...
                                  <span className="text-xs font-medium text-slate-500 ml-0.5">{item.unit}</span>
                                </span>
                              </div>
              ...
              >
                <div className="flex justify-between items-center mb-5 border-b border-slate-100 pb-3">
                  <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2 uppercase tracking-wide">
                    <span className="bg-emerald-100 text-emerald-700 p-1 rounded-md text-xs px-2 border border-emerald-200 font-black">
                      ฿
                    </span>{' '}
                    การเงิน & ค่าใช้จ่าย
                  </h3>
            ...
            <div
              className={`bg-white p-6 rounded-xl border shadow-sm relative z-0 transition-all ${
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <div className="flex gap-3 mb-2 relative z-20">
                    <select
                      className="w-1/3 border border-slate-300 rounded-lg p-2.5 bg-slate-50 text-sm font-medium focus:ring-primary focus:border-primary"
                      value={referenceType}
                    ...
                    <div className="w-2/3">
                      <SearchableSelect
                        ...
                        options={fetchedJobs.map((j) => {
                          ...
                          return {
                            value: j.id,
                            label: `[${jobDate}] ${c ? `${c.first_name} ${c.last_name}` : 'Unknown'}`,
                          };
                        })}
