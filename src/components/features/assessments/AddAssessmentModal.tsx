...
interface AddAssessmentModalProps {
  ...
  onClose: () => void;
  onCreateAssessment: (
    assessmentData: Omit<Assessment, 'id' | 'code'> & {
...
export const AddAssessmentModal: FC<AddAssessmentModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const fetchData = async () => {
        try {
          const [customersRes, packagesRes, productsRes, categoriesRes] =
            await Promise.all([
  ...
  const handleAddInstallment = () => {
    setInstallments((prev) => {
      ...
      const baseAmount =
        Math.floor((totalEstimatedCost / newCount) * 100) / 100;
        ...
        <div className="relative after:absolute after:inset-x-0 after:top-1/2 after:block after:h-0.5 after:-translate-y-1/2 after:rounded-lg after:bg-slate-100">
          <ol className="relative z-10 flex justify-between text-sm font-medium text-slate-500">
            {STEPS.map((step, index) => {
              const isCompleted =
                visitedSteps.includes(index) && currentStep > index;
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-slate-500">
                  จัดการพื้นที่และเลือกแพ็กเกจบริการ
                </p>
              </div>
            </div>
                        ...
                        <table className="min-w-full divide-y divide-slate-200">
                          ...
                          <tfoot className="bg-slate-50">
                            <tr>
                              <td
                                colSpan={2}
                                className="px-4 py-2 text-right text-xs font-bold text-slate-600"
                              >
                                รวม
                              </td>
                              <td
                      ...
                      {Math.abs(
                        ...
                        <p className="text-xs text-red-500 text-right">
                          * ยอดรวมงวดงานต้องเท่ากับยอดรวมสุทธิ (
                          {totalEstimatedCost.toLocaleString()} บาท)
                        </p>
                      )}
                    </div>
                  )}
                  ...
                  <div className="p-4 space-y-4">
                    ...
                    <div className="space-y-2">
                      {workAreas.map((area, idx) => (
                        <div
                          key={idx}
                          className="flex justify-between text-xs text-slate-500 pl-2 border-l-2 border-slate-100"
                        >
                          <span className="truncate max-w-[150px]">
                            {area.area_name}
                          </span>
                          <span>
                            ฿{(area.total_price || 0).toLocaleString()}
                          </span>
                        </div>
                      ))}
                    </div>
