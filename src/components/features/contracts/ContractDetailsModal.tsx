...
export const ContractDetailsModal: React.FC<ContractDetailsModalProps> = ({
  ...
  useEffect(() => {
    if (contract.quotation_id) {
      QuotationApi.getById(contract.quotation_id)
        .then((res) => {
  ...
  return (
    <Modal isOpen={true} onClose={onClose} title="รายละเอียดใบสัญญา" size="4xl">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center p-6 border-b border-slate-100 bg-slate-50/50">
          ...
          <div className="mt-4 md:mt-0 text-right">
            <p className="text-sm text-slate-500 mb-1">
              มูลค่าสัญญา (Total Amount)
            </p>
            <p className="text-3xl font-bold text-green-600">
              ฿
              {(Number(contract.total_amount) || 0).toLocaleString('th-TH', {
                ...
                <div className="bg-slate-50 p-3 rounded-md border border-slate-100 flex items-center gap-2">
                  ...
                  <span className="text-sm text-slate-600">
                    ...
                    <span className="font-medium text-slate-900">
                      {(quotation || fullQuotation).code ||
                        `QT-${(quotation || fullQuotation).id.slice(0, 8)}`}
                    </span>
                  </span>
                </div>
              )}
              ...
              <div className="mb-8 overflow-hidden rounded-lg border border-slate-200">
                <div className="bg-slate-50 px-6 py-3 border-b border-slate-200 flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-semibold text-slate-700">
                    รายละเอียดพื้นที่ (Area Breakdown)
                  </h3>
                </div>
                            ...
                            <div className="bg-slate-50 p-3 rounded-lg border border-slate-200">
                              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                                {serviceTypeOptions.map((option) => {
                                  const isChecked =
                                    area.category_services?.some((cat: any) => {
                                      const matchId =
                                        (cat.category_id &&
                                          cat.category_id === option.id) ||
                                        (cat.category?.id &&
                                          cat.category.id === option.id);
                                      const matchName =
                                        (cat.name &&
                                          cat.name === option.value) ||
                                        (cat.category?.name &&
                                          cat.category.name === option.value);
