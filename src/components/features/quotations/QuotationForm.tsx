...
export const QuotationForm: React.FC<QuotationFormProps> = ({
    ...
    useEffect(() => {
        if (selectedAssessment && mode === 'create' && !initialValues?.items) {
            // Set customer
    ...
    // ^ Note: This might not trigger perfectly if just switching assessments in Edit mode. 
    // But usually we don't switch assessments in Edit mode easily without expecting changes.
    ...
    // Auto-fill customer info - only if not already set or specifically changed
    useEffect(() => {
        if (selectedCustomer && (!serviceLocation || mode === 'create')) { // heuristic
            // Reuse address logic
            const address = [
            ...
            // Only set if empty or forcing update
            if (!serviceLocation) {
                setServiceLocation(address);
            }
        }
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <FormField label="ประเภทบริการ" htmlFor="service-type">
                        ...
                        >
                            ...
                            <option value="ควบคุมป้องกันกำจัดปลวก">ควบคุมป้องกันกำจัดปลวก</option>
                            <option value="ควบคุมป้องกันกำจัดมด">ควบคุมป้องกันกำจัดมด</option>
                            <option value="ควบคุมป้องกันกำจัดแมลงสาบ">ควบคุมป้องกันกำจัดแมลงสาบ</option>
                            <option value="ควบคุมป้องกันกำจัดหนู">ควบคุมป้องกันกำจัดหนู</option>
                            <option value="ควบคุมป้องกันกำจัดปลวก มด แมลงสาบ หนู">ควบคุมป้องกันกำจัดปลวก มด แมลงสาบ หนู</option>
                        </Select>
                    </FormField>
            ...
            <div className="flex justify-end gap-3">
                ...
                <Button type="submit" variant="primary" className="px-6">
                    {mode === 'create' ? 'สร้างใบเสนอราคา' : mode === 'edit' ? 'บันทึกการแก้ไข' : 'สร้างฉบับแก้ไข'}
                </Button>
            </div>
        </form>
    );
