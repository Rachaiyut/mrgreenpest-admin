  ...
  return (
    ...
    >
      <div className="space-y-4 text-sm">
        ...
        <DetailsList cols={2}>
          <DetailsItem label="เลขที่ใบเสนอราคา" valueClassName="font-semibold">
            {quotation.id}
          </DetailsItem>
                ...
                <tbody className="bg-white divide-y divide-slate-200">
                  {quotation.installments.map((inst) => (
                    <tr key={inst.id}>
                      ...
                      <td className="px-3 py-2 text-sm text-right text-slate-900 font-medium">
                        ฿
                      ...
                      <td className="px-3 py-2 text-sm text-center">
                        {inst.status === Status.Pending ? (
                          <button
                            onClick={() =>
                              onCreateInvoice && onCreateInvoice(inst)
                            }
                  ...
                  <li key={item.id} className="py-4">
                    <div className="flex items-center space-x-4">
                      <div className="flex-shrink-0">
                        <span
                          className={`inline-flex items-center justify-center h-8 w-8 rounded-full ${item.id === quotation.id ? 'bg-primary text-white' : 'bg-slate-100 text-slate-500'}`}
                        >
