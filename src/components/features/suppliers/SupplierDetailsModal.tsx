  ...
  return (
    ...
    >
      <div className="space-y-4 text-sm">
        ...
        <DetailsList cols={2}>
            <DetailsItem label="รหัสผู้จัดจำหน่าย" valueClassName="font-semibold">{supplier.id}</DetailsItem>
            <DetailsItem label="ชื่อผู้จัดจำหน่าย" valueClassName="font-semibold">{supplier.name}</DetailsItem>
            <DetailsItem label="ประเภท">{supplier.type}</DetailsItem>
            {supplier.taxId && (<DetailsItem label="เลขประจำตัวผู้เสียภาษี">{supplier.taxId}</DetailsItem>)}
            {supplier.contactPerson && (<DetailsItem label="ผู้ติดต่อ">{supplier.contactPerson}</DetailsItem>)}
            <DetailsItem label="เบอร์โทรศัพท์">
                <ul className="space-y-1">
                    {supplier.phones.map((phone, index) => (
                        <li key={index}>{phone}{index === 0 && ' (หลัก)'}</li>
                    ))}
                </ul>
            </DetailsItem>
