...
export const AddSupplierModal: React.FC<AddSupplierModalProps> = ({ isOpen, onClose, onCreateSupplier, suppliers }) => {
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!data['supplier-name'] || !phones[0].trim() || (supplierType === 'นิติบุคคล' && !data['tax-id'])) {
        ...
        <Modal 
            ...
            footer={
                <div className="flex gap-2">
                    ...
                    <button type="submit" form="add-supplier-form" className="py-2 px-4 rounded-lg bg-primary hover:bg-primary/90 text-white font-semibold shadow-sm">
                        บันทึก
                    </button>
                </div>
            }
        >
            <form id="add-supplier-form" ref={formRef} onSubmit={handleSubmit} className="space-y-4">
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <FormField label="รหัสผู้จัดจำหน่าย" htmlFor="supplier-id">
                        <Input id="supplier-id" type="text" value={generatedId} readOnly className="bg-slate-100" />
                    </FormField>
                    <FormField label="ประเภทผู้จัดจำหน่าย">
                        <div className="flex rounded-lg bg-slate-100 p-1 w-full">
                            <label className="relative flex-1 cursor-pointer">
                                <input type="radio" name="supplierType" value="บุคคลธรรมดา" className="sr-only peer" checked={supplierType === 'บุคคลธรรมดา'} onChange={() => setSupplierType('บุคคลธรรมดา')} />
                            ...
                            <label className="relative flex-1 cursor-pointer">
                                <input type="radio" name="supplierType" value="นิติบุคคล" className="sr-only peer" checked={supplierType === 'นิติบุคคล'} onChange={() => setSupplierType('นิติบุคคล')} />
                ...
                <FormField label={supplierType === 'นิติบุคคล' ? 'ชื่อบริษัท' : 'ชื่อ-นามสกุล'} htmlFor="supplier-name">
                    <Input name="supplier-name" id="supplier-name" type="text" required />
                </FormField>

                <FormField label="เลขประจำตัวผู้เสียภาษี" htmlFor="tax-id">
                    <Input name="tax-id" id="tax-id" type="text" required={supplierType === 'นิติบุคคล'} />
                </FormField>
