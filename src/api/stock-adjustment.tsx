...
class StockAdjustmentService extends AuthService {
  ...
  async getAll(query?: IBaseQuery): Promise<IBaseResponseArray<StockAdjustment>> {
    const res = await this.http.get<IBaseResponseArray<StockAdjustment>>(this.path, { params: query });
    return res.data;
  }
  ...
  async update(id: string, data: Partial<StockAdjustment>): Promise<StockAdjustment> {
    const res = await this.http.patch<StockAdjustment>(`${this.path}/${id}`, data);
    return res.data;
  }
