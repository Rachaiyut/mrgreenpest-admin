...
class ReturnToSupplierService extends AuthService {
  ...
  async getAll(
    query?: IBaseQuery
  ): Promise<IBaseResponseArray<ReturnToSupplier>> {
    const res = await this.http.get<IBaseResponseArray<ReturnToSupplier>>(
      this.path,
      { params: query }
    );
    return res.data;
  ...
  async update(
    id: string,
    data: Partial<ReturnToSupplier>
  ): Promise<ReturnToSupplier> {
    const res = await this.http.patch<ReturnToSupplier>(
      `${this.path}/${id}`,
      data
    );
    return res.data;
