...
class ContractService extends AuthService {
  ...
  async generateSigningLink(customerId: string, contractId: string, expiresHours: number = 72): Promise<{ token: string; expires_at: string }> {
    const res = await this.http.post<{ data: { token: string; expires_at: string } }>(`/customer/${customerId}/contract/${contractId}/signing-link`, { expires_hours: expiresHours });
