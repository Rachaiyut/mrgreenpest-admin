...
import { IBaseQuery, IBaseResponse, IBaseResponseArray } from '@/src/types/entity/base.interface';
...
class TransferService extends AuthService {
  ...
  async getAll(query?: IBaseQuery): Promise<IBaseResponseArray<Transfer>> {
    const res = await this.http.get<IBaseResponseArray<Transfer>>(
      this.path,
      { params: query }
    );
    return res.data;
  ...
  async create(data: Omit<Transfer, 'id' | 'createdAt' | 'updatedAt'>): Promise<Transfer> {
  ...
  async updateStatus(id: string, status: string): Promise<Transfer> {
    const res = await this.http.patch<IBaseResponse<Transfer>>(`${this.path}/${id}/status`, { status });
    return res.data.data;
  }
}
