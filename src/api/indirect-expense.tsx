...
class IndirectExpenseService extends AuthService {
  ...
  async getAll(query?: IBaseQuery): Promise<IBaseResponseArray<IndirectExpense>> {
    const res = await this.http.get<IBaseResponseArray<IndirectExpense>>(this.path, { params: query });
    return res.data;
  }
  ...
  async update(id: string, data: Partial<IndirectExpense>): Promise<IndirectExpense> {
    const res = await this.http.patch<IndirectExpense>(`${this.path}/${id}`, data);
    return res.data;
  }
