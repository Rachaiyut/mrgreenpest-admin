...
class ProductReturnService extends AuthService {
  ...
  async getAll(query?: IBaseQuery): Promise<IBaseResponseArray<ProductReturn>> {
    const res = await this.http.get<IBaseResponseArray<ProductReturn>>(this.path, { params: query });
    return res.data;
  }
  ...
  async update(id: string, data: Partial<ProductReturn>): Promise<ProductReturn> {
    const res = await this.http.patch<ProductReturn>(`${this.path}/${id}`, data);
    return res.data;
  }
