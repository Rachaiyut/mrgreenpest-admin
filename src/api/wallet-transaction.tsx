...
class WalletTransactionService extends AuthService {
  ...
  async getAll(query?: IBaseQuery): Promise<IBaseResponseArray<WalletTransaction>> {
    const res = await this.http.get<IBaseResponseArray<WalletTransaction>>(this.path, { params: query });
    return res.data;
  }
  ...
  async create(data: Omit<WalletTransaction, 'id'>): Promise<WalletTransaction> {
  ...
  async update(id: string, data: Partial<WalletTransaction>): Promise<WalletTransaction> {
    const res = await this.http.patch<WalletTransaction>(`${this.path}/${id}`, data);
    return res.data;
  }
