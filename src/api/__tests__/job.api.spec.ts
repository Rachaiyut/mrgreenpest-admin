...
describe('JobApi', () => {
  it('should be defined', () => { expect(JobApi).toBeDefined(); });
  describe('getAll', () => {
    it('should call GET /jobs', async () => {
      const mockGet = vi.fn().mockResolvedValue({ data: { data: [], meta: { total: 0 } } });
      (JobApi as any).http = { get: mockGet, interceptors: { request: { use: vi.fn() }, response: { use: vi.fn() } } };
      await JobApi.getAll({ page: 1, limit: 10 } as any);
