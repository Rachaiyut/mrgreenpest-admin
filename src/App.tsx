...
import { Page, Assessment, FieldJob, Status, Customer, Contract, Quotation, Product, User, Warehouse as WarehouseType, Supplier, GoodsReceipt as GoodsReceiptType, Withdrawal as WithdrawalType, Transfer as TransferType, StockAdjustment as StockAdjustmentType, ProductReturn as ProductReturnType, Invoice, Receipt, Category, UserWallet, WalletTransaction, ReturnToSupplier as ReturnToSupplierType } from './types';
...
const App: React.FC = () => {
  ...
  const PATH_PAGE = Object.fromEntries(Object.entries(PAGE_PATH).map(([k, v]) => ['/' + v, k])) as Record<string, Page>;
  ...
  if (!PATH_PAGE[location.pathname]) {
    ...
    else if (location.pathname.startsWith('/billing')) currentPage = 'ใบแจ้งหนี้';
    else if (location.pathname.startsWith('/reports/monthly-sales')) currentPage = 'ยอดขาย(รายเดือน)';
    else if (location.pathname.startsWith('/reports/sales-summary')) currentPage = 'สรุปยอดขาย(รายเดือน)';
    else if (location.pathname.startsWith('/reports/tax-invoice-income')) currentPage = 'รายได้ออกใบกำกับ(รายเดือน)';
    else if (location.pathname.startsWith('/reports/indirect-expenses')) currentPage = 'ค่าใช้จ่ายทางอ้อม';
    else if (location.pathname.startsWith('/reports/daily-cash')) currentPage = 'บัญชีเงินสดรายวัน';
    else if (location.pathname.startsWith('/reports/direct-expenses')) currentPage = 'ค่าใช้จ่ายทางตรง';
    else if (location.pathname.startsWith('/receipts')) currentPage = 'ใบกำกับภาษี/ใบเสร็จรับเงิน';
  ...
  const goToReports = useCallback((tab: string) => {
    try { localStorage.setItem('reportsDefaultTab', tab); } catch { }
    navigate('/reports');
  }, [navigate]);
  ...
  const createWarehouse = useCallback((data: Omit<WarehouseType, 'id'>) => {
    setWarehouses(prev => {
      ...
      const maxId = prev.reduce((max, w) => {
        ...
        } else if (w.id.startsWith('wh-')) { // Handle old format
  ...
  const createQuotation = useCallback((data: Omit<Quotation, 'id'>, assessmentId?: string) => {
    setQuotations(prev => {
      ...
      const maxRunning = runningNumbers.length > 0 ? Math.max(...runningNumbers) : 0;
      ...
      if (assessmentId) {
        setAssessments(prevAssessments =>
          prevAssessments.map(asm =>
            asm.id === assessmentId ? { ...asm, status: Status.Converted } : asm
          )
        );
      }
      return [...prev, { ...data, assessmentId, id: newId, revision: 1 } as Quotation];
    });
  }, []);
  ...
  const createJob = useCallback((data: Omit<FieldJob, 'id'>, assessmentId?: string) => {
    setFieldJobs(prev => {
      const thaiYearLastTwoDigits = (new Date().getFullYear() + 543).toString().slice(-2);
  ...
  const createWalletTransaction = useCallback((userId: string, transactionData: Omit<WalletTransaction, 'id'>) => {
    setUserWallets(prev => {
      const userWalletIndex = prev.findIndex(w => w.userId === userId);
      const newTxn = { ...transactionData, id: `txn-${Date.now()}` } as WalletTransaction;
  ...
  const updateWarehouseLimits = useCallback((warehouseId: string, limits: { [productId: string]: number }) => {
    setWarehouses(prev => prev.map(w => w.id === warehouseId ? { ...w, withdrawalLimits: limits } : w));
  }, []);
  ...
  const handleReviseQuotation = useCallback((quotation: Quotation) => {
    setQuotations(prev => {
      ...
      const maxVersion = versions.length > 0 ? Math.max(...versions) : (currentVersionStr ? parseInt(currentVersionStr, 10) : 0);
  ...
  const HANDLERS = {
    ...
    categories: { create: createCategory, update: updateHandler(setCategories), delete: deleteHandler(setCategories) },
  ...
  const renderPage = () => {
    switch (currentPage) {
      case 'Dashboard':
        ...
        />;
      case 'ลูกค้า':
        return <Customers customers={customers} onCreateCustomer={HANDLERS.customers.create} onUpdateCustomer={HANDLERS.customers.update} onDeleteCustomer={HANDLERS.customers.delete} contracts={contracts} quotations={quotations} onCreateContract={HANDLERS.contracts.create} />;
      case 'ใบประเมิน':
        return <Assessments assessments={assessments} onCreateAssessment={HANDLERS.assessments.create} onUpdateAssessment={HANDLERS.assessments.update} onDeleteAssessment={HANDLERS.assessments.delete} products={products} />;
      case 'ภาคสนาม':
        ...
        />;
      case 'ใบเสนอราคา':
        return <Financials defaultTab="ใบเสนอราคา" quotations={quotations} customers={customers} assessments={assessments} onCreateQuotation={HANDLERS.quotations.create} onUpdateQuotation={HANDLERS.quotations.update} onDeleteQuotation={HANDLERS.quotations.delete} onReviseQuotation={HANDLERS.quotations.revise} />;
      case 'ใบแจ้งหนี้':
        return <Financials
      ...
      case 'ใบกำกับภาษี/ใบเสร็จรับเงิน':
        return <Financials defaultTab="ใบกำกับภาษี/ใบเสร็จรับเงิน" receipts={receipts} invoices={invoices} customers={customers} onCreateReceipt={HANDLERS.receipts.create} onUpdateReceipt={HANDLERS.receipts.update} onDeleteReceipt={HANDLERS.receipts.delete} />;
      case 'หมวดหมู่':
        return <Categories categories={categories} onCreateCategory={HANDLERS.categories.create} onUpdateCategory={HANDLERS.categories.update} onDeleteCategory={HANDLERS.categories.delete} />;
      case 'สินค้า/บริการ':
        return <Inventory products={products} onCreateProduct={HANDLERS.products.create} onUpdateProduct={HANDLERS.products.update} onDeleteProduct={HANDLERS.products.delete} categories={categories} />;
      case 'แพ็กเกจ':
        return <Packages products={products} onCreatePackage={HANDLERS.packages.create} onUpdatePackage={HANDLERS.packages.update} onDeletePackage={HANDLERS.packages.delete} categories={categories} />;
      case 'คลังสินค้า':
        return <Warehouse warehouses={warehouses} products={products} onCreateWarehouse={HANDLERS.warehouses.create} onUpdateWarehouse={HANDLERS.warehouses.update} onDeleteWarehouse={HANDLERS.warehouses.delete} onUpdateWarehouseLimits={HANDLERS.warehouses.updateLimits} stockMap={warehouseStocks} />;
      case 'ผู้จัดจำหน่าย':
        return <Suppliers suppliers={suppliers} onCreateSupplier={HANDLERS.suppliers.create} onUpdateSupplier={HANDLERS.suppliers.update} onDeleteSupplier={HANDLERS.suppliers.delete} />;
      case 'รับเข้า':
        return <GoodsReceipt receipts={goodsReceipts} onCreateReceipt={HANDLERS.goodsReceipts.create} onUpdateReceipt={HANDLERS.goodsReceipts.update} onDeleteReceipt={HANDLERS.goodsReceipts.delete} warehouses={warehouses} suppliers={suppliers} products={products} />;
      case 'เบิกสินค้า/อุปกรณ์ และค่าใช้จ่าย':
        ...
        />;
      case 'โอนย้าย':
        return <Transfers transfers={transfers} onCreateTransfer={HANDLERS.transfers.create} onUpdateTransfer={HANDLERS.transfers.update} onDeleteTransfer={HANDLERS.transfers.delete} warehouses={warehouses} products={products} stockMap={warehouseStocks} />;
      case 'ปรับปรุง Stock':
        return <StockAdjustment adjustments={stockAdjustments} onCreateAdjustment={HANDLERS.stockAdjustments.create} onUpdateAdjustment={HANDLERS.stockAdjustments.update} onDeleteAdjustment={HANDLERS.stockAdjustments.delete} warehouses={warehouses} products={products} stockMap={warehouseStocks} />;
      case 'คืนสินค้า':
        return <Returns returns={productReturns} onCreateReturn={HANDLERS.productReturns.create} onUpdateReturn={HANDLERS.productReturns.update} onDeleteReturn={HANDLERS.productReturns.delete} warehouses={warehouses} products={products} stockMap={warehouseStocks} />;
      case 'เบิกสินค้าคืนผู้จำหน่าย':
        return <ReturnToSupplier returns={returnToSuppliers} onCreateReturn={HANDLERS.returnToSuppliers.create} onUpdateReturn={HANDLERS.returnToSuppliers.update} onDeleteReturn={HANDLERS.returnToSuppliers.delete} warehouses={warehouses} suppliers={suppliers} products={products} />;
      case 'ผู้ใช้งาน':
        return <Users users={users} onCreateUser={HANDLERS.users.create} onUpdateUser={HANDLERS.users.update} onDeleteUser={HANDLERS.users.delete} userWallets={userWallets} onCreateWalletTransaction={HANDLERS.userWallets.createTransaction} />;
      case 'จัดการบทบาท':
        return <Users users={users} onCreateUser={HANDLERS.users.create} onUpdateUser={HANDLERS.users.update} onDeleteUser={HANDLERS.users.delete} defaultView='roles' userWallets={userWallets} onCreateWalletTransaction={HANDLERS.userWallets.createTransaction} />;
      case 'การแจ้งเตือน':
        return <Notifications contracts={contracts} jobs={fieldJobs} invoices={invoices} receipts={receipts} customers={customers} />;
      case 'รายงาน':
        return <Reports
          products={products}
          warehouses={warehouses}
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-slate-100">
      <Sidebar currentPage={currentPage} onPageChange={(page) => navigate('/' + PAGE_PATH[page])} isOpen={isSidebarOpen} toggleSidebar={toggleSidebarState} />
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <React.Suspense fallback={<div className="p-8"><p className="text-slate-600">กำลังโหลด...</p></div>}>
              <Routes>
                ...
                <Route path="/customers" element={<Customers customers={customers} onCreateCustomer={HANDLERS.customers.create} onUpdateCustomer={HANDLERS.customers.update} onDeleteCustomer={HANDLERS.customers.delete} contracts={contracts} quotations={quotations} onCreateContract={HANDLERS.contracts.create} onCreateJob={HANDLERS.fieldJobs.create} />} />
                <Route path="/assessments" element={<Assessments assessments={assessments} onCreateAssessment={HANDLERS.assessments.create} onUpdateAssessment={HANDLERS.assessments.update} onDeleteAssessment={HANDLERS.assessments.delete} products={products} />} />
                <Route path="/field-operations" element={<FieldOperations users={users} jobs={fieldJobs} assessments={assessments} contracts={contracts} quotations={quotations} onCreateJob={HANDLERS.fieldJobs.create} onUpdateJob={HANDLERS.fieldJobs.update} onDeleteJob={HANDLERS.fieldJobs.delete} products={products} onUpdateAssessment={HANDLERS.assessments.update} onUpdateQuotation={HANDLERS.quotations.update} customers={customers} onCreateQuotation={HANDLERS.quotations.create} />} />
                <Route path="/quotations/new" element={<CreateQuotationPage onCreateQuotation={HANDLERS.quotations.create} customers={customers} assessments={assessments} />} />
                <Route path="/quotations/:id/edit" element={<EditQuotationPage quotations={quotations} onUpdateQuotation={HANDLERS.quotations.update} />} />
                <Route path="/invoices/:id/edit" element={<EditInvoicePage invoices={invoices} customers={customers} quotations={quotations} onUpdateInvoice={HANDLERS.invoices.update} />} />
                <Route path="/quotations" element={<Financials defaultTab="ใบเสนอราคา" quotations={quotations} customers={customers} assessments={assessments} onCreateQuotation={HANDLERS.quotations.create} onUpdateQuotation={HANDLERS.quotations.update} onDeleteQuotation={HANDLERS.quotations.delete} onReviseQuotation={HANDLERS.quotations.revise} />} />
                <Route path="/billing" element={<Financials defaultTab="ใบแจ้งหนี้" invoices={invoices} customers={customers} quotations={quotations} onCreateInvoice={HANDLERS.invoices.create} onUpdateInvoice={HANDLERS.invoices.update} onDeleteInvoice={HANDLERS.invoices.delete} onCreateReceipt={HANDLERS.receipts.create} />} />
                <Route path="/receipts" element={<Financials defaultTab="ใบกำกับภาษี/ใบเสร็จรับเงิน" receipts={receipts} invoices={invoices} customers={customers} onCreateReceipt={HANDLERS.receipts.create} onUpdateReceipt={HANDLERS.receipts.update} onDeleteReceipt={HANDLERS.receipts.delete} />} />
                <Route path="/categories" element={<Categories categories={categories} onCreateCategory={HANDLERS.categories.create} onUpdateCategory={HANDLERS.categories.update} onDeleteCategory={HANDLERS.categories.delete} />} />
                <Route path="/inventory" element={<Inventory products={products} onCreateProduct={HANDLERS.products.create} onUpdateProduct={HANDLERS.products.update} onDeleteProduct={HANDLERS.products.delete} categories={categories} />} />
                <Route path="/packages" element={<Packages products={products} onCreatePackage={HANDLERS.packages.create} onUpdatePackage={HANDLERS.packages.update} onDeletePackage={HANDLERS.packages.delete} categories={categories} />} />
                <Route path="/warehouse" element={<Warehouse warehouses={warehouses} products={products} onCreateWarehouse={HANDLERS.warehouses.create} onUpdateWarehouse={HANDLERS.warehouses.update} onDeleteWarehouse={HANDLERS.warehouses.delete} onUpdateWarehouseLimits={HANDLERS.warehouses.updateLimits} stockMap={warehouseStocks} />} />
                <Route path="/suppliers" element={<Suppliers suppliers={suppliers} onCreateSupplier={HANDLERS.suppliers.create} onUpdateSupplier={HANDLERS.suppliers.update} onDeleteSupplier={HANDLERS.suppliers.delete} />} />
                <Route path="/reporting/monthly-sales" element={<MonthlySales quotations={quotations} assessments={assessments} users={users} />} />
                <Route path="/reports/monthly-sales" element={<MonthlySales quotations={quotations} assessments={assessments} users={users} />} />
                <Route path="/reports/sales-summary" element={<SalesSummary quotations={quotations} assessments={assessments} fieldJobs={fieldJobs} />} />
                <Route path="/goods-receipt" element={<GoodsReceipt receipts={goodsReceipts} onCreateReceipt={HANDLERS.goodsReceipts.create} onUpdateReceipt={HANDLERS.goodsReceipts.update} onDeleteReceipt={HANDLERS.goodsReceipts.delete} warehouses={warehouses} suppliers={suppliers} products={products} />} />
                <Route path="/withdrawals" element={<Withdrawals withdrawals={withdrawals} onCreateWithdrawal={HANDLERS.withdrawals.create} onUpdateWithdrawal={HANDLERS.withdrawals.update} onDeleteWithdrawal={HANDLERS.withdrawals.delete} users={users} warehouses={warehouses} jobs={fieldJobs} customers={customers} currentUser={users[0]} products={products} stockMap={warehouseStocks} />} />
                <Route path="/transfers" element={<Transfers transfers={transfers} onCreateTransfer={HANDLERS.transfers.create} onUpdateTransfer={HANDLERS.transfers.update} onDeleteTransfer={HANDLERS.transfers.delete} warehouses={warehouses} products={products} stockMap={warehouseStocks} />} />
                <Route path="/stock-adjustment" element={<StockAdjustment adjustments={stockAdjustments} onCreateAdjustment={HANDLERS.stockAdjustments.create} onUpdateAdjustment={HANDLERS.stockAdjustments.update} onDeleteAdjustment={HANDLERS.stockAdjustments.delete} warehouses={warehouses} products={products} stockMap={warehouseStocks} />} />
                <Route path="/returns" element={<Returns returns={productReturns} onCreateReturn={HANDLERS.productReturns.create} onUpdateReturn={HANDLERS.productReturns.update} onDeleteReturn={HANDLERS.productReturns.delete} warehouses={warehouses} products={products} stockMap={warehouseStocks} />} />
                <Route path="/return-to-supplier" element={<ReturnToSupplier returns={returnToSuppliers} onCreateReturn={HANDLERS.returnToSuppliers.create} onUpdateReturn={HANDLERS.returnToSuppliers.update} onDeleteReturn={HANDLERS.returnToSuppliers.delete} warehouses={warehouses} suppliers={suppliers} products={products} />} />
                <Route path="/users" element={<Users users={users} onCreateUser={HANDLERS.users.create} onUpdateUser={HANDLERS.users.update} onDeleteUser={HANDLERS.users.delete} userWallets={userWallets} onCreateWalletTransaction={HANDLERS.userWallets.createTransaction} />} />
                <Route path="/roles" element={<Users users={users} onCreateUser={HANDLERS.users.create} onUpdateUser={HANDLERS.users.update} onDeleteUser={HANDLERS.users.delete} defaultView='roles' userWallets={userWallets} onCreateWalletTransaction={HANDLERS.userWallets.createTransaction} />} />
                <Route path="/notifications" element={<Notifications contracts={contracts} jobs={fieldJobs} invoices={invoices} receipts={receipts} customers={customers} />} />
                <Route path="/reports" element={<Reports products={products} warehouses={warehouses} suppliers={suppliers} users={users} stockMap={warehouseStocks} goodsReceipts={goodsReceipts} withdrawals={withdrawals} transfers={transfers} stockAdjustments={stockAdjustments} productReturns={productReturns} fieldJobs={fieldJobs} invoices={invoices} receipts={receipts} customers={customers} />} />
                <Route path="/reports/total-income" element={<TotalIncome invoices={invoices} receipts={receipts} customers={customers} />} />
                <Route path="/reports/tax-invoice-income" element={<TaxInvoiceIncome invoices={invoices} receipts={receipts} customers={customers} />} />
                <Route path="/reports/indirect-expenses" element={<IndirectExpenses />} />
